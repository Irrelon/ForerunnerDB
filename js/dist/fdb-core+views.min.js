...
!1,g=!1,l,k,m,n,p;for(n in h)if(h.hasOwnProperty(n)){l=!1;if("$"===n.substr(0,1))switch(n){case "$inc":l=!0;for(p in h[n])if(h[n].hasOwnProperty(p)&&"$"!==p.substr(0,1))if("number"===typeof d[p])this._updateIncrement(d,p,h[n][p]),c=!0;else throw"Cannot increment field that is not a number! ("+p+")!";break;case "$push":l=!0;for(p in h[n])if(h[n].hasOwnProperty(p)&&"$"!==p.substr(0,1))if(d[p]instanceof Array)this._updatePush(d[p],h[n][p]),c=!0;else throw"Cannot push to a key that is not an array! ("+
p+")!";break;case "$addToSet":l=!0;for(p in h[n])if(h[n].hasOwnProperty(p)&&"$"!==p.substr(0,1))if(d[p]instanceof Array){k=d[p];var q;m=k.length;var u,g=!0;q=a&&a.$addToSet;var s,v;q&&q.key?(s=!1,v=new e(q.key),u=v.value(h[n][p])[0]):(u=JSON.stringify(h[n][p]),s=!0);for(q=0;q<m;q++)if(s){if(JSON.stringify(k[q])===u){g=!1;break}}else if(u===v.value(k[q])[0]){g=!1;break}g&&(this._updatePush(d[p],h[n][p]),c=!0)}else throw"Cannot push to a key that is not an array! ("+p+")!";break;case "$splicePush":l=
!0;for(p in h[n])if(h[n].hasOwnProperty(p)&&"$"!==p.substr(0,1))if(d[p]instanceof Array)if(c=h[n].$index,void 0!==c)delete h[n][p].$index,this._updateSplicePush(d[p],c,h[n][p]),c=!0;else throw"Cannot splicePush without a $index integer value!";else throw"Cannot splicePush with a key that is not an array! ("+p+")!";break;case "$move":l=!0;for(p in h[n])if(h[n].hasOwnProperty(p)&&"$"!==p.substr(0,1))if(d[p]instanceof Array)for(m=0;m<d[p].length;m++){if(this._match(d[p][m],h[n][p])){c=h[n].$index;if(void 0!==
c)this._updateSpliceMove(d[p],m,c),c=!0;else throw"Cannot move without a $index integer value!";break}}else throw"Cannot pull from a key that is not an array! ("+p+")!";break;case "$pull":for(p in l=!0,h[n])if(h[n].hasOwnProperty(p)&&"$"!==p.substr(0,1))if(d[p]instanceof Array){k=[];for(m=0;m<d[p].length;m++)this._match(d[p][m],h[n][p])&&k.push(m);for(m=k.length;m--;)this._updatePull(d[p],k[m]),c=!0}else throw"Cannot pull from a key that is not an array! ("+p+")!";}if(".$"===n.substr(n.length-2,2)&&
(l=!0,n=n.substr(0,n.length-2),g=new e(f+"."+n),d[n]&&d[n]instanceof Array&&d[n].length)){k=[];for(m=0;m<d[n].length;m++)this._match(d[n][m],g.value(b)[0])&&k.push(m);for(m=0;m<k.length;m++)(g=this._updateObject(d[n][k[m]],h[n+".$"],b,a,f+"."+n))&&(c=!0)}if(!l)if("object"===typeof h[n])if(null!==d[n]&&"object"===typeof d[n])if(l=d[n]instanceof Array,k=h[n]instanceof Array,l||k)if(!k&&l)for(m=0;m<d[n].length;m++)(g=this._updateObject(d[n][m],h[n],b,a,f+"."+n))&&(c=!0);else this._updateProperty(d,n,
h[n]),c=!0;else(g=this._updateObject(d[n],h[n],b,a,f+"."+n))&&(c=!0);else this._updateProperty(d,n,h[n]),c=!0;else d[n]!==h[n]&&(this._updateProperty(d,n,h[n]),c=!0)}return c};f.prototype._updateProperty=function(d,h,b){this._linked?($.observable(d).setProperty(h,b),this.debug()&&console.log('ForerunnerDB.Collection: Setting data-bound document property "'+h+'" for collection "'+this.name()+'"')):(d[h]=b,this.debug()&&console.log('ForerunnerDB.Collection: Setting non-data-bound document property "'+
...
function(d,h){var b={};b[this._primaryKey]=d;return this.find(b,h)[0]};f.prototype.peek=function(d,b){var a=this._data,e=a.length,c,g,l=new f;if("string"===typeof d){for(c=0;c<e;c++)g=JSON.stringify(a[c]),-1<g.indexOf(d)&&l.insert(a[c]);return l.find({},b)}return this.find(d,b)};f.prototype.explain=function(d,b){return this.find(d,b).__fdbOp._data};f.prototype.find=function(d,b){d=d||{};b=b||{};b.decouple=void 0!==b.decouple?b.decouple:!0;var a=this._metrics.create("find"),f=this,c,g=!0,l,k,m,q,n,
p,w,u,s,v=[];k=function(b){return f._match(b,d,"and")};a.start();if(d){a.time("analyseQuery");c=this._analyseQuery(d,b,a);a.time("analyseQuery");a.data("analysis",c);if(c.hasJoin&&c.queriesJoin){a.time("joinReferences");for(m=0;m<c.joinsOn.length;m++)n=c.joinsOn[m],q=new e(c.joinQueries[n]),q=q.value(d)[0],this._db.collection(c.joinsOn[m]).subset(q);a.time("joinReferences")}c.indexMatch.length&&(!b||b&&!b.skipIndex)?(a.data("index.potential",c.indexMatch),a.data("index.used",c.indexMatch[0].index),
...
l=this.decouple(l),a.time("decouple"),a.data("flag.decouple",!0));if(b.join){for(k=0;k<b.join.length;k++)for(n in b.join[k])if(b.join[k].hasOwnProperty(n))for(u=n,c=this._db.collection(n),g=b.join[k][n],s=0;s<l.length;s++){w={};q=m=!1;for(p in g)if(g.hasOwnProperty(p))if("$"===p.substr(0,1))switch(p){case "$as":u=g[p];break;case "$multi":m=g[p];break;case "$require":q=g[p];break;default:p.substr(0,3)}else w[p]=(new e(g[p])).value(l[s])[0];w=c.find(w);!q||q&&w[0]?l[s][u]=!1===m?w[0]:w:v.push(l[s])}a.data("flag.join",
!0)}if(v.length){a.time("removalQueue");for(p=0;p<v.length;p++)n=l.indexOf(v[p]),-1<n&&l.splice(n,1);a.time("removalQueue")}if(b.transform){a.time("transform");for(p=0;p<l.length;p++)l.splice(p,1,b.transform(l[p]));a.time("transform");a.data("flag.transform",!0)}this._transformEnabled&&this._transformOut&&(a.time("transformOut"),l=this.transformOut(l),a.time("transformOut"));a.data("results",l.length);a.stop()}else a.stop(),l=[];l.__fdbOp=a;return l};f.prototype.transform=function(d){return void 0!==
...
f.prototype.bucket=function(d,b){var a,e={};for(a=0;a<b.length;a++)e[b[a][d]]=e[b[a][d]]||[],e[b[a][d]].push(b[a]);return e};f.prototype._analyseQuery=function(d,b,a){var c={queriesOn:[this._name],indexMatch:[],hasJoin:!1,queriesJoin:!1,joinQueries:{},query:d,options:b},f,g=[],l=[],k,m,q,n,p;a.time("checkIndexes");void 0!==d[this._primaryKey]&&(a.time("checkIndexMatch: Primary Key"),k=new e,c.indexMatch.push({lookup:this._primaryIndex.lookup(d,b),keyData:{matchedKeys:[this._primaryKey],matchedKeyCount:1,
totalKeyCount:k.countKeys(d)},index:this._primaryIndex}),a.time("checkIndexMatch: Primary Key"));for(p in this._indexById)if(this._indexById.hasOwnProperty(p)&&(m=this._indexById[p],q=m.name(),a.time("checkIndexMatch: "+q),k=m.match(d,b),n=m.lookup(d,b),0<k.matchedKeyCount&&c.indexMatch.push({lookup:n,keyData:k,index:m}),a.time("checkIndexMatch: "+q),k.totalKeyCount===k.matchedKeyCount))break;a.time("checkIndexes");1<c.indexMatch.length&&(a.time("findOptimalIndex"),c.indexMatch.sort(function(b,d){return b.keyData.totalKeyCount===
b.keyData.matchedKeyCount?-1:d.keyData.totalKeyCount===d.keyData.matchedKeyCount?1:b.keyData.matchedKeyCount===d.keyData.matchedKeyCount?b.lookup.length-d.lookup.length:d.keyData.matchedKeyCount-b.keyData.matchedKeyCount}),a.time("findOptimalIndex"));if(b.join){c.hasJoin=!0;for(a=0;a<b.join.length;a++)for(f in b.join[a])b.join[a].hasOwnProperty(f)&&(g.push(f),"$as"in b.join[a][f]?l.push(b.join[a][f].$as):l.push(f));for(f=0;f<l.length;f++)if(b=this._queryReferencesCollection(d,l[f],""))c.joinQueries[g[f]]=
b,c.queriesJoin=!0;c.joinsOn=g;c.queriesOn=c.queriesOn.concat(g)}return c};f.prototype._queryReferencesCollection=function(b,a,e){for(var c in b)if(b.hasOwnProperty(c)){if(c===a)return e&&(e+="."),e+c;if("object"===typeof b[c])return e&&(e+="."),e+=c,this._queryReferencesCollection(b[c],a,e)}return!1};f.prototype._match=function(b,a,e){var c,f;c=typeof b;f=typeof a;var g=!0,l;if(!("string"!==c&&"number"!==c||"string"!==f&&"number"!==f))b!==a&&(g=!1);else for(l in a)if(a.hasOwnProperty(l)){c=!1;if("$"===
