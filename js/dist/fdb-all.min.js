!function(Pa){if("object"==typeof exports&&"undefined"!=typeof module)module.exports=Pa();else if("function"==typeof define&&define.amd)define([],Pa);else{var e;"undefined"!=typeof window?e=window:"undefined"!=typeof global?e=global:"undefined"!=typeof self&&(e=self);e.ForerunnerDB=Pa()}}(function(){return function e(m,k,h){function a(c,b){if(!k[c]){if(!m[c]){var f="function"==typeof require&&require;if(!b&&f)return f(c,!0);if(d)return d(c,!0);f=Error("Cannot find module '"+c+"'");throw f.code="MODULE_NOT_FOUND",
f;}f=k[c]={exports:{}};m[c][0].call(f.exports,function(f){var d=m[c][1][f];return a(d?d:f)},f,f.exports,e,m,k,h)}return k[c].exports}for(var d="function"==typeof require&&require,b=0;b<h.length;b++)a(h[b]);return a}({1:[function(e,m,k){k=e("../lib/Core");e("../lib/CollectionGroup");e("../lib/View");e("../lib/Highcharts");e("../lib/Persist");e("../lib/Document");e("../lib/Overview");e("../lib/vendor/jsviews");m.exports=k},{"../lib/CollectionGroup":3,"../lib/Core":4,"../lib/Document":6,"../lib/Highcharts":7,
"../lib/Overview":12,"../lib/Persist":14,"../lib/View":16,"../lib/vendor/jsviews":17}],2:[function(e,m,k){var h,a,d,b,c;k=e("./Shared");var g=function(f){this.init.apply(this,arguments)};g.prototype.init=function(f){this._primaryKey="_id";this._primaryIndex=new a("primary");this._primaryCrc=new a("primaryCrc");this._crcLookup=new a("crcLookup");this._name=f;this._data=[];this._groups=[];this._metrics=new h;this._linked=0;this._deferQueue={insert:[],update:[],remove:[],upsert:[]};this._deferThreshold=
{insert:100,update:100,remove:100,upsert:100};this._deferTime={insert:1,update:1,remove:1,upsert:1};this._subsetOf(this)};k.addModule("Collection",g);k.inherit(g.prototype,k.chainSystem);h=e("./Metrics");a=e("./KeyValueStore");d=e("./Path");b=e("./Index");c=e("./Crc");e=k.modules.Core;g.prototype.debug=k.common.debug;g.prototype.crc=c;k.synthesize(g.prototype,"name");g.prototype.on=k.common.on;g.prototype.off=k.common.off;g.prototype.emit=k.common.emit;g.prototype.drop=function(){if(this._db&&this._db._collection&&
this._name){this.debug()&&console.log("Dropping collection "+this._name);this.emit("drop");delete this._db._collection[this._name];var f=[],c;for(c=0;c<this._groups.length;c++)f.push(this._groups[c]);for(c=0;c<f.length;c++)this._groups[c].removeCollection(this);return!0}return!1};g.prototype.primaryKey=function(f){return void 0!==f?(this._primaryKey!==f&&(this._primaryKey=f,this._primaryIndex.primaryKey(f),this._rebuildPrimaryKeyIndex()),this):this._primaryKey};g.prototype._onInsert=function(f,c){this.emit("insert",
f,c)};g.prototype._onUpdate=function(f){this.emit("update",f)};g.prototype._onRemove=function(f){this.emit("remove",f)};k.synthesize(g.prototype,"db");g.prototype.setData=function(f,c,a){if(f){var d=this._metrics.create("setData");d.start();c=c||{};c.$decouple=void 0!==c.$decouple?c.$decouple:!0;c.$decouple&&(f=this.decouple(f));f instanceof Array||(f=[f]);d.time("transformIn");f=this.transformIn(f);d.time("transformIn");var b=this._data;this._linked?this.remove():this._data=[];f.length&&(this._linked?
this.insert(f):this._data=this._data.concat(f));d.time("Rebuild Primary Key Index");this._rebuildPrimaryKeyIndex(c);d.time("Rebuild Primary Key Index");d.time("Resolve chains");this.chainSend("setData",f,{oldData:b});d.time("Resolve chains");d.stop();this.emit("setData",this._data,b)}a&&a(!1);return this};g.prototype._rebuildPrimaryKeyIndex=function(f){var c=f&&void 0!==f.ensureKeys?f.ensureKeys:!0;f=f&&void 0!==f.violationCheck?f.violationCheck:!0;var d,a,b,g=this._primaryIndex,h=this._primaryCrc,
e=this._crcLookup,k=this._primaryKey,q;g.truncate();h.truncate();e.truncate();d=this._data;for(a=d.length;a--;)if(b=d[a],c&&this._ensurePrimaryKey(b),f){if(!g.uniqueSet(b[k],b))throw"Call to setData failed because your data violates the primary key unique constraint. One or more documents are using the same primary key: "+b[this._primaryKey];}else q=JSON.stringify(b),g.set(b[k],b),h.set(b[k],q),e.set(q,b)};g.prototype._ensurePrimaryKey=function(f){void 0===f[this._primaryKey]&&(f[this._primaryKey]=
this.objectId())};g.prototype.truncate=function(){this.emit("truncate",this._data);this._data.length=0;this.deferEmit("change");return this};g.prototype.upsert=function(f,c){if(f){var d=this._deferQueue.upsert,a=this._deferThreshold.upsert,b={},g;if(f instanceof Array){if(f.length>a){this._deferQueue.upsert=d.concat(f);this.processQueue("upsert",c);return}b=[];for(d=0;d<f.length;d++)b.push(this.upsert(f[d]));c&&c();return b}f[this._primaryKey]?(g={},g[this._primaryKey]=f[this._primaryKey],this.count(g)?
b.op="update":b.op="insert"):b.op="insert";switch(b.op){case "insert":b.result=this.insert(f);break;case "update":b.result=this.update(g,f)}return b}c&&c()};g.prototype.update=function(f,c,d){c=this.decouple(c);c=this.transformIn(c);this.debug()&&console.log('Updating some collection data for collection "'+this.name()+'"');var b=this,a=this._metrics.create("update"),g=this._primaryKey,h,e,k=function(a){if(c&&void 0!==c[g]&&c[g]!=a[g]){b._primaryIndex.unSet(a[g]);var h=b._updateObject(a,c,f,d,"");
if(b._primaryIndex.uniqueSet(a[g],a))return h;throw"Primary key violation in update! Key violated: "+a[g];}return b._updateObject(a,c,f,d,"")};a.start();a.time("Retrieve documents to update");h=this.find(f,{$decouple:!1});a.time("Retrieve documents to update");h.length&&(a.time("Update documents"),e=h.filter(k),a.time("Update documents"),e.length&&(a.time("Resolve chains"),this.chainSend("update",{query:f,update:c},d),a.time("Resolve chains"),this._onUpdate(e),this.deferEmit("change",{type:"update",
data:e})));a.stop();return e||[]};g.prototype.updateById=function(f,c){var a={};a[this._primaryKey]=f;return this.update(a,c)};g.prototype._updateObject=function(f,c,a,b,g,h){c=this.decouple(c);g=g||"";"."===g.substr(0,1)&&(g=g.substr(1,g.length-1));var e=!1,k=!1,n,q,p,l;for(l in c)if(c.hasOwnProperty(l)){n=!1;if("$"===l.substr(0,1))switch(l){case "$index":break;default:n=!0,(k=this._updateObject(f,c[l],a,b,g,l))&&(e=!0)}if(this._isPositionalKey(l)&&(n=!0,l=l.substr(0,l.length-2),k=new d(g+"."+l),
f[l]&&f[l]instanceof Array&&f[l].length)){q=[];for(p=0;p<f[l].length;p++)this._match(f[l][p],k.value(a)[0])&&q.push(p);for(p=0;p<q.length;p++)(k=this._updateObject(f[l][q[p]],c[l+".$"],a,b,g+"."+l,h))&&(e=!0)}if(!n)if(h||"object"!==typeof c[l])switch(h){case "$inc":this._updateIncrement(f,l,c[l]);e=!0;break;case "$push":void 0===f[l]&&(f[l]=[]);if(f[l]instanceof Array){if(void 0!==c[l].$position&&c[l].$each instanceof Array)for(k=c[l].$position,n=c[l].$each.length,p=0;p<n;p++)this._updateSplicePush(f[l],
k+p,c[l].$each[p]);else if(c[l].$each instanceof Array)for(n=c[l].$each.length,p=0;p<n;p++)this._updatePush(f[l],c[l].$each[p]);else this._updatePush(f[l],c[l]);e=!0}else throw"Cannot push to a key that is not an array! ("+l+")";break;case "$pull":if(f[l]instanceof Array){q=[];for(p=0;p<f[l].length;p++)this._match(f[l][p],c[l])&&q.push(p);for(n=q.length;n--;)this._updatePull(f[l],q[n]),e=!0}break;case "$pullAll":if(f[l]instanceof Array)if(c[l]instanceof Array){if(q=f[l],n=q.length,0<n)for(;n--;){for(k=
0;k<c[l].length;k++)q[n]===c[l][k]&&(this._updatePull(f[l],n),n--,e=!0);if(0>n)break}}else throw"Cannot pullAll without being given an array of values to pull! ("+l+")";break;case "$addToSet":void 0===f[l]&&(f[l]=[]);if(f[l]instanceof Array){p=f[l];var m;q=p.length;var F;n=!0;m=b&&b.$addToSet;var aa;m&&m.key?(k=!1,aa=new d(m.key),F=aa.value(c[l])[0]):(F=JSON.stringify(c[l]),k=!0);for(m=0;m<q;m++)if(k){if(JSON.stringify(p[m])===F){n=!1;break}}else if(F===aa.value(p[m])[0]){n=!1;break}n&&(this._updatePush(f[l],
c[l]),e=!0)}else throw"Cannot addToSet on a key that is not an array! (undefined)!";break;case "$splicePush":void 0===f[l]&&(f[l]=[]);if(f[l]instanceof Array)if(k=c.$index,void 0!==k)delete c.$index,this._updateSplicePush(f[l],k,c[l]),e=!0;else throw"Cannot splicePush without a $index integer value!";else throw"Cannot splicePush with a key that is not an array! ("+l+")";break;case "$move":if(f[l]instanceof Array)for(p=0;p<f[l].length;p++){if(this._match(f[l][p],c[l])){e=c[l].$index;if(void 0!==e)this._updateSpliceMove(f[l],
p,e),e=!0;else throw"Cannot move without a $index integer value!";break}}else throw"Cannot move on a key that is not an array! ("+l+")";break;case "$mul":this._updateMultiply(f,l,c[l]);e=!0;break;case "$rename":this._updateRename(f,l,c[l]);e=!0;break;case "$unset":this._updateUnset(f,l);e=!0;break;case "$pop":if(f[l]instanceof Array)this._updatePop(f[l],c[l])&&(e=!0);else throw"Cannot pop from a key that is not an array! ("+l+")";break;default:f[l]!==c[l]&&(this._updateProperty(f,l,c[l]),e=!0)}else if(null!==
f[l]&&"object"===typeof f[l])if(p=f[l]instanceof Array,q=c[l]instanceof Array,p||q)if(!q&&p)for(p=0;p<f[l].length;p++)(k=this._updateObject(f[l][p],c[l],a,b,g+"."+l,h))&&(e=!0);else f[l]!==c[l]&&(this._updateProperty(f,l,c[l]),e=!0);else(k=this._updateObject(f[l],c[l],a,b,g+"."+l,h))&&(e=!0);else f[l]!==c[l]&&(this._updateProperty(f,l,c[l]),e=!0)}return e};g.prototype._isPositionalKey=function(f){return".$"===f.substr(f.length-2,2)};g.prototype._updateProperty=function(f,c,a){this._linked?(jQuery.observable(f).setProperty(c,
a),this.debug()&&console.log('ForerunnerDB.Collection: Setting data-bound document property "'+c+'" for collection "'+this.name()+'"')):(f[c]=a,this.debug()&&console.log('ForerunnerDB.Collection: Setting non-data-bound document property "'+c+'" for collection "'+this.name()+'"'))};g.prototype._updateIncrement=function(f,c,a){this._linked?jQuery.observable(f).setProperty(c,f[c]+a):f[c]+=a};g.prototype._updateSpliceMove=function(f,c,a){this._linked?(jQuery.observable(f).move(c,a),this.debug()&&console.log('ForerunnerDB.Collection: Moving data-bound document array index from "'+
c+'" to "'+a+'" for collection "'+this.name()+'"')):(f.splice(a,0,f.splice(c,1)[0]),this.debug()&&console.log('ForerunnerDB.Collection: Moving non-data-bound document array index from "'+c+'" to "'+a+'" for collection "'+this.name()+'"'))};g.prototype._updateSplicePush=function(f,c,a){f.length>c?this._linked?jQuery.observable(f).insert(c,a):f.splice(c,0,a):this._linked?jQuery.observable(f).insert(a):f.push(a)};g.prototype._updatePush=function(f,c){this._linked?jQuery.observable(f).insert(c):f.push(c)};
g.prototype._updatePull=function(f,c){this._linked?jQuery.observable(f).remove(c):f.splice(c,1)};g.prototype._updateMultiply=function(f,c,a){this._linked?jQuery.observable(f).setProperty(c,f[c]*a):f[c]*=a};g.prototype._updateRename=function(f,c,a){var b=f[c];this._linked?(jQuery.observable(f).setProperty(a,b),jQuery.observable(f).removeProperty(c)):(f[a]=b,delete f[c])};g.prototype._updateUnset=function(f,c){this._linked?jQuery.observable(f).removeProperty(c):delete f[c]};g.prototype._updatePop=function(f,
c){var a,b=!1;0<f.length&&(this._linked?(1===c?a=f.length-1:-1===c&&(a=0),-1<a&&(jQuery.observable(arr).remove(a),b=!0)):1===c?(f.pop(),b=!0):-1===c&&(f.shift(),b=!0));return b};g.prototype.remove=function(f,c,a){var b,d;if(f instanceof Array){d=[];for(a=0;a<f.length;a++)d.push(this.remove(f[a],{noEmit:!0}));(!c||c&&!c.noEmit)&&this._onRemove(d);return d}a=this.find(f,{$decouple:!1});if(a.length){for(d=0;d<a.length;d++)b=a[d],this._removeIndex(b),b=this._data.indexOf(b),this._linked?jQuery.observable(this._data).remove(b):
this._data.splice(b,1);this.chainSend("remove",{query:f},c);(!c||c&&!c.noEmit)&&this._onRemove(a);this.deferEmit("change",{type:"remove",data:a})}return a};g.prototype.removeById=function(f){var c={};c[this._primaryKey]=f;return this.remove(c)};g.prototype.deferEmit=function(){var f=this,c;this._noEmitDefer||this._db&&(!this._db||this._db._noEmitDefer)?this.emit.apply(this,arguments):(c=arguments,this._changeTimeout&&clearTimeout(this._changeTimeout),this._changeTimeout=setTimeout(function(){f.debug()&&
console.log("ForerunnerDB.Collection: Emitting "+c[0]);f.emit.apply(f,c)},100))};g.prototype.processQueue=function(c,a){var b=this._deferQueue[c],d=this._deferThreshold[c],g=this._deferTime[c];if(b.length){var e=this;b.length&&(b=b.length>d?b.splice(0,d):b.splice(0,b.length),this[c](b));setTimeout(function(){e.processQueue(c,a)},g)}else a&&a()};g.prototype.insert=function(c,a,b){"function"===typeof a?(b=a,a=this._data.length):void 0===a&&(a=this._data.length);c=this.transformIn(c);return this._insertHandle(c,
a,b)};g.prototype._insertHandle=function(c,a,b){var d=this._deferQueue.insert,g=this._deferThreshold.insert,e=[],h=[];if(c instanceof Array){if(c.length>g){this._deferQueue.insert=d.concat(c);this.processQueue("insert",b);return}for(g=0;g<c.length;g++)d=this._insert(c[g],a+g),!0===d?e.push(c[g]):h.push({doc:c[g],reason:d})}else d=this._insert(c,a),!0===d?e.push(c):h.push({doc:c,reason:d});this.chainSend("insert",c,{index:a});this._onInsert(e,h);b&&b();this.deferEmit("change",{type:"insert",data:e});
return{inserted:e,failed:h}};g.prototype._insert=function(c,a){if(c){var b;this._ensurePrimaryKey(c);if(b=this.insertIndexViolation(c))return"Index violation in index: "+b;this._insertIndex(c);this._linked?jQuery.observable(this._data).insert(a,c):this._data.splice(a,0,c);return!0}return"No document passed to insert"};g.prototype._insertIndex=function(c){var a=this._indexByName,b,d=JSON.stringify(c);this._primaryIndex.uniqueSet(c[this._primaryKey],c);this._primaryCrc.uniqueSet(c[this._primaryKey],
d);this._crcLookup.uniqueSet(d,c);for(b in a)a.hasOwnProperty(b)&&a[b].insert(c)};g.prototype._removeIndex=function(c){var a=this._indexByName,b,d=JSON.stringify(c);this._primaryIndex.unSet(c[this._primaryKey]);this._primaryCrc.unSet(c[this._primaryKey]);this._crcLookup.unSet(d);for(b in a)a.hasOwnProperty(b)&&a[b].remove(c)};g.prototype.subset=function(c,a){var b=this.find(c,a);return(new g)._subsetOf(this).primaryKey(this._primaryKey).setData(b)};g.prototype.subsetOf=function(){return this.__subsetOf};
g.prototype._subsetOf=function(c){this.__subsetOf=c;return this};g.prototype.distinct=function(c,a,b){a=this.find(a,b);c=new d(c);b={};var g=[],e,h;for(h=0;h<a.length;h++)(e=c.value(a[h])[0])&&!b[e]&&(b[e]=!0,g.push(e));return g};g.prototype.decouple=k.common.decouple;g.prototype.findById=function(c,a){var b={};b[this._primaryKey]=c;return this.find(b,a)[0]};g.prototype.peek=function(c,a){var b=this._data,d=b.length,e,h,k=new g;if("string"===typeof c){for(e=0;e<d;e++)h=JSON.stringify(b[e]),-1<h.indexOf(c)&&
k.insert(b[e]);return k.find({},a)}return this.find(c,a)};g.prototype.explain=function(c,a){return this.find(c,a).__fdbOp._data};g.prototype.find=function(c,a){c=c||{};a=a||{};a.$decouple=void 0!==a.$decouple?a.$decouple:!0;var b=this._metrics.create("find"),g=this,e,h=!0,k,A,n,q,p,l,m,F,aa,la=[];A=function(a){return g._match(a,c,"and")};b.start();if(c){b.time("analyseQuery");e=this._analyseQuery(c,a,b);b.time("analyseQuery");b.data("analysis",e);if(e.hasJoin&&e.queriesJoin){b.time("joinReferences");
...
b.time("transform");b.data("flag.transform",!0)}this._transformEnabled&&this._transformOut&&(b.time("transformOut"),k=this.transformOut(k),b.time("transformOut"));b.data("results",k.length);b.stop()}else b.stop(),k=[];k.__fdbOp=b;return k};g.prototype.indexOf=function(c){if(c=this.find(c,{$decouple:!1})[0])return this._data.indexOf(c)};g.prototype.transform=function(c){return void 0!==c?("object"===typeof c?(void 0!==c.enabled&&(this._transformEnabled=c.enabled),void 0!==c.dataIn&&(this._transformIn=
c.dataIn),void 0!==c.dataOut&&(this._transformOut=c.dataOut)):this._transformEnabled=!1===c?!1:!0,this):{enabled:this._transformEnabled,dataIn:this._transformIn,dataOut:this._transformOut}};g.prototype.transformIn=function(c){if(this._transformEnabled&&this._transformIn){if(c instanceof Array){var a=[],b;for(b=0;b<c.length;b++)a[b]=this._transformIn(c[b]);return a}return this._transformIn(c)}return c};g.prototype.transformOut=function(c){if(this._transformEnabled&&this._transformOut){if(c instanceof
Array){var a=[],b;for(b=0;b<c.length;b++)a[b]=this._transformOut(c[b]);return a}return this._transformOut(c)}return c};g.prototype.sort=function(c,a){a=a||[];var b=[],d,g;for(d in c)c.hasOwnProperty(d)&&(g={},g[d]=c[d],g.___fdbKey=d,b.push(g));return 2>b.length?this._sort(c,a):this._bucketSort(b,a)};g.prototype._bucketSort=function(c,a){var b=c.shift(),d,g,e=[];if(0<c.length){a=this._sort(b,a);d=this.bucket(b.___fdbKey,a);for(g in d)d.hasOwnProperty(g)&&(b=[].concat(c),e=e.concat(this._bucketSort(b,
d[g])));return e}return this._sort(b,a)};g.prototype._sort=function(c,a){var b=new d,g=b.parse(c,!0)[0];b.path(g.path);return a.sort(1===g.value?function(c,a){var d=b.value(c)[0],f=b.value(a)[0];return"string"===typeof d&&"string"===typeof f?d.localeCompare(f):d>f?1:d<f?-1:0}:function(c,a){var d=b.value(c)[0],f=b.value(a)[0];return"string"===typeof d&&"string"===typeof f?1===d.localeCompare(f)?-1:1:d>f?-1:d<f?1:0})};g.prototype.bucket=function(c,a){var b,d={};for(b=0;b<a.length;b++)d[a[b][c]]=d[a[b][c]]||
[],d[a[b][c]].push(a[b]);return d};g.prototype._analyseQuery=function(c,a,b){var g={queriesOn:[this._name],indexMatch:[],hasJoin:!1,queriesJoin:!1,joinQueries:{},query:c,options:a},e,h=[],k=[],m,n,q,p,l;b.time("checkIndexes");void 0!==c[this._primaryKey]&&(b.time("checkIndexMatch: Primary Key"),m=new d,g.indexMatch.push({lookup:this._primaryIndex.lookup(c,a),keyData:{matchedKeys:[this._primaryKey],matchedKeyCount:1,totalKeyCount:m.countKeys(c)},index:this._primaryIndex}),b.time("checkIndexMatch: Primary Key"));
for(l in this._indexById)if(this._indexById.hasOwnProperty(l)&&(n=this._indexById[l],q=n.name(),b.time("checkIndexMatch: "+q),m=n.match(c,a),p=n.lookup(c,a),0<m.matchedKeyCount&&g.indexMatch.push({lookup:p,keyData:m,index:n}),b.time("checkIndexMatch: "+q),m.totalKeyCount===m.matchedKeyCount))break;b.time("checkIndexes");1<g.indexMatch.length&&(b.time("findOptimalIndex"),g.indexMatch.sort(function(c,a){return c.keyData.totalKeyCount===c.keyData.matchedKeyCount?-1:a.keyData.totalKeyCount===a.keyData.matchedKeyCount?
1:c.keyData.matchedKeyCount===a.keyData.matchedKeyCount?c.lookup.length-a.lookup.length:a.keyData.matchedKeyCount-c.keyData.matchedKeyCount}),b.time("findOptimalIndex"));if(a.join){g.hasJoin=!0;for(b=0;b<a.join.length;b++)for(e in a.join[b])a.join[b].hasOwnProperty(e)&&(h.push(e),"$as"in a.join[b][e]?k.push(a.join[b][e].$as):k.push(e));for(e=0;e<k.length;e++)if(a=this._queryReferencesCollection(c,k[e],""))g.joinQueries[h[e]]=a,g.queriesJoin=!0;g.joinsOn=h;g.queriesOn=g.queriesOn.concat(h)}return g};
g.prototype._queryReferencesCollection=function(c,a,b){for(var d in c)if(c.hasOwnProperty(d)){if(d===a)return b&&(b+="."),b+d;if("object"===typeof c[d])return b&&(b+="."),b+=d,this._queryReferencesCollection(c[d],a,b)}return!1};g.prototype._match=function(c,a,b){var d,g;d=typeof c;g=typeof a;var e=!0,h;if(!("string"!==d&&"number"!==d||"string"!==g&&"number"!==g))c!==a&&(e=!1);else for(h in a)if(a.hasOwnProperty(h)){d=!1;if("$"===h.substr(0,1))switch(h){case "$gt":if(c>a[h]){if("or"===b)return!0}else e=
!1;d=!0;break;case "$gte":if(c>=a[h]){if("or"===b)return!0}else e=!1;d=!0;break;case "$lt":if(c<a[h]){if("or"===b)return!0}else e=!1;d=!0;break;case "$lte":if(c<=a[h]){if("or"===b)return!0}else e=!1;d=!0;break;case "$exists":if(void 0===c!==a[h]){if("or"===b)return!0}else e=!1;d=!0;break;case "$or":d=!0;for(g=0;g<a[h].length;g++){if(this._match(c,a[h][g],"and"))return!0;e=!1}break;case "$and":d=!0;for(g=0;g<a[h].length;g++)if(!this._match(c,a[h][g],"and"))return!1;break;case "$in":if(a[h]instanceof
Array){d=a[h];g=d.length;var k,m=!1;for(k=0;k<g;k++)if(d[k]===c){m=!0;break}if(m){if("or"===b)return!0}else e=!1}else throw"Cannot use a $nin operator on a non-array key: "+h;d=!0;break;case "$nin":if(a[h]instanceof Array){d=a[h];g=d.length;m=!0;for(k=0;k<g;k++)if(d[k]===c){m=!1;break}if(m){if("or"===b)return!0}else e=!1}else throw"Cannot use a $nin operator on a non-array key: "+h;d=!0;break;case "$ne":if(c!=a[h]){if("or"===b)return!0}else e=!1;d=!0}if(!d&&a[h]instanceof RegExp)if(d=!0,"object"===
typeof c&&void 0!==c[h]&&a[h].test(c[h])){if("or"===b)return!0}else e=!1;if(!d)if("object"===typeof a[h])if(void 0!==c[h]){if(c[h]instanceof Array&&!(a[h]instanceof Array))for(d=!1,g=0;g<c[h].length&&!(d=this._match(c[h][g],a[h],void 0));g++);else if(!(c[h]instanceof Array)&&a[h]instanceof Array)for(d=!1,g=0;g<a[h].length&&!(d=this._match(c[h],a[h][g],void 0));g++);else d="object"===typeof c?this._match(c[h],a[h],void 0):this._match(void 0,a[h],void 0);if(d){if("or"===b)return!0}else e=!1}else if(a[h]&&
void 0!==a[h].$exists)if(d=this._match(void 0,a[h],void 0)){if("or"===b)return!0}else e=!1;else e=!1;else if(c&&c[h]===a[h]){if("or"===b)return!0}else if(c&&c[h]&&c[h]instanceof Array&&a[h]&&"object"!==typeof a[h]){d=!1;for(g=0;g<c[h].length&&!(d=this._match(c[h][g],a[h],void 0));g++);if(d){if("or"===b)return!0}else e=!1}else e=!1;if("and"===b&&!e)return!1}return e};g.prototype.count=function(c,a){return c?this.find(c,a).length:this._data.length};g.prototype.link=function(c,a){if(window.jQuery){this._links=
...
this.name()+'" to output target: '+c);return this}throw'Cannot bind view data to output target selector "'+c+'" because it does not exist in the DOM!';}throw"Cannot create a duplicate link to the target: "+c+" with the template: "+b;}throw"Cannot data-bind without jQuery, please add jQuery to your page!";};g.prototype.unlink=function(c,a){if(window.jQuery){this._links=this._links||{};var b;a&&"object"===typeof a?a.template&&"string"===typeof a.template&&(b=this.objectId(a.template)):b=a;if(this._links[b])return jQuery.templates[b].unlink(c),
delete this._links[b],this._linked--,this.debug()&&console.log('ForerunnerDB.Collection: Removed binding collection "'+this.name()+'" to output target: '+c),this;console.log("Cannot remove link, one does not exist to the target: "+c+" with the template: "+a)}else throw"Cannot data-bind without jQuery, please add jQuery to your page!";};g.prototype.findSub=function(c,a,b,g){var h=new d(a);c=this.find(c);var e=c.length,k,m,n=this._db.collection("__FDB_temp_"+this.objectId()),q={parents:e,subDocTotal:0,
subDocs:[],pathFound:!1,err:""};for(k=0;k<e;k++)if(m=h.value(c[k])[0]){n.setData(m);m=n.find(b,g);if(g.returnFirst&&m.length)return m[0];q.subDocs.push(m);q.subDocTotal+=m.length;q.pathFound=!0}n.drop();if(g.noStats)return q.subDocs;q.pathFound||(q.err="No objects found in the parent documents with a matching path of: "+a);return q};g.prototype.insertIndexViolation=function(c){var a,b=this._indexByName,d,g;if(this._primaryIndex.get(c[this._primaryKey]))a=this._primaryIndex;else for(d in b)if(b.hasOwnProperty(d)&&
(g=b[d],g.unique()&&g.violation(c))){a=g;break}return a?a.name():!1};g.prototype.ensureIndex=function(c,a){this._indexByName=this._indexByName||{};this._indexById=this._indexById||{};var d=new b(c,a,this),g={start:(new Date).getTime()};if(this._indexByName[d.name()])return{err:"Index with that name already exists"};if(this._indexById[d.id()])return{err:"Index with those keys already exists"};d.rebuild();this._indexByName[d.name()]=d;this._indexById[d.id()]=d;g.end=(new Date).getTime();g.total=g.end-
g.start;this._lastOp={type:"ensureIndex",stats:{time:g}};return{index:d,id:d.id(),name:d.name(),state:d.state()}};g.prototype.index=function(c){if(this._indexByName)return this._indexByName[c]};g.prototype.lastOp=function(){return this._metrics.list()};g.prototype.objectId=k.common.objectId;g.prototype.diff=function(c){var a={insert:[],update:[],remove:[]},b=this.primaryKey(),d,g,h,e;if(b===c.primaryKey()){d=c._data;e=d.length;for(g=0;g<e;g++)h=d[g],this._primaryIndex.get(h[b])?this._primaryCrc.get(h[b])!==
c._primaryCrc.get(h[b])&&a.update.push(h):a.insert.push(h);d=this._data;e=d.length;for(g=0;g<e;g++)h=d[g],c._primaryIndex.get(h[b])||a.remove.push(h)}return a};e.prototype.collection=function(c,a){if(c)return this._collection[c]||this.debug()&&console.log("Creating collection "+c),this._collection[c]=this._collection[c]||(new g(c)).db(this),void 0!==a&&this._collection[c].primaryKey(a),this._collection[c];throw"Cannot get collection with undefined name!";};e.prototype.collectionExists=function(c){return Boolean(this._collection[c])};
e.prototype.collections=function(){var c=[],a;for(a in this._collection)this._collection.hasOwnProperty(a)&&c.push({name:a,count:this._collection[a].count()});return c};m.exports=g},{"./Crc":5,"./Index":8,"./KeyValueStore":9,"./Metrics":10,"./Path":13,"./Shared":15}],3:[function(e,m,k){var h,a;k=e("./Shared");var d=function(){this.init.apply(this,arguments)};d.prototype.init=function(b){this._name=b;this._data=new a("__FDB__cg_data_"+this._name);this._collections=[];this._views=[]};k.addModule("CollectionGroup",
...
c&&a._groups.splice(c,1));0===this._collections.length&&delete this._primaryKey}return this};d.prototype.decouple=k.common.decouple;d.prototype._chainHandler=function(a,c,d,f){switch(c){case "setData":d=this.decouple(d);this._data.remove(f.oldData);this._data.insert(d);break;case "insert":d=this.decouple(d);this._data.insert(d);break;case "update":this._data.update(d.query,d.update,f);break;case "remove":this._data.remove(d.query,f)}};d.prototype.insert=function(){this._collectionsRun("insert",arguments)};
...
function(a){return a?(this._collectionGroup[a]=this._collectionGroup[a]||(new d(a)).db(this),this._collectionGroup[a]):this._collectionGroup};m.exports=d},{"./Collection":2,"./Shared":15}],4:[function(e,m,k){var h;k=e("./Shared.js");var a=function(){this.init.apply(this,arguments)};a.prototype.init=function(){this._collection={};this._debug={}};a.shared=k;a.prototype.shared=k;k.addModule("Core",a);k.inherit(a.prototype,k.chainSystem);h=e("./Collection.js");e("./Metrics.js");e=e("./Crc.js");a.prototype._isServer=
!1;a.prototype.isClient=function(){return!this._isServer};a.prototype.isServer=function(){return this._isServer};a.prototype.crc=e;a.prototype.isClient=function(){return!this._isServer};a.prototype.isServer=function(){return this._isServer};a.prototype.decouple=k.common.decouple;a.prototype.debug=k.common.debug;a.prototype.arrayToCollection=function(a){return(new h).setData(a)};a.prototype.on=function(a,b){this._listeners=this._listeners||{};this._listeners[a]=this._listeners[a]||[];this._listeners[a].push(b);
return this};a.prototype.off=function(a,b){if(a in this._listeners){var c=this._listeners[a],g=c.indexOf(b);-1<g&&c.splice(g,1)}return this};a.prototype.emit=function(a,b){this._listeners=this._listeners||{};if(a in this._listeners){var c=this._listeners[a],g=c.length,f;for(f=0;f<g;f++)c[f].apply(this,Array.prototype.slice.call(arguments,1))}return this};a.prototype.objectId=k.common.objectId;a.prototype.peek=function(a){var b,c,g=[],f=typeof a;for(b in this._collection)this._collection.hasOwnProperty(b)&&
(c=this._collection[b],g="string"===f?g.concat(c.peek(a)):g.concat(c.find(a)));return g};a.prototype.peekCat=function(a){var b,c,g={},f,h=typeof a;for(b in this._collection)this._collection.hasOwnProperty(b)&&(c=this._collection[b],(f="string"===h?c.peek(a):c.find(a))&&f.length&&(g[c.name()]=f));return g};m.exports=a},{"./Collection.js":2,"./Crc.js":5,"./Metrics.js":10,"./Shared.js":15}],5:[function(e,m,k){var h=function(){var a=[],d,b,c;for(b=0;256>b;b++){d=b;for(c=0;8>c;c++)d=d&1?3988292384^d>>>
...
d):a[c]*=d};d.prototype._updateRename=function(a,c,d){var f=a[c];this._linked?(jQuery.observable(a).setProperty(d,f),jQuery.observable(a).removeProperty(c)):(a[d]=f,delete a[c])};d.prototype._updateUnset=function(a,c){this._linked?jQuery.observable(a).removeProperty(c):delete a[c]};d.prototype._updatePop=function(a,c){var d,f=!1;0<a.length&&(this._linked?(1===c?d=a.length-1:-1===c&&(d=0),-1<d&&(jQuery.observable(arr).remove(d),f=!0)):1===c?(a.pop(),f=!0):-1===c&&(a.shift(),f=!0));return f};d.prototype.link=
...
b){this._options=b;this._selector=$(this._options.selector);this._listeners={};this._collection=a;this._options.series=[];var c,g;switch(this._options.type){case "pie":this._selector.highcharts(this._options.chartOptions);this._chart=this._selector.highcharts();c=this._collection.find();g={allowPointSelect:!0,cursor:"pointer",dataLabels:{enabled:!0,format:"<b>{point.name}</b>: {y} ({point.percentage:.0f}%)",style:{color:Highcharts.theme&&Highcharts.theme.contrastTextColor||"black"}}};c=this.pieDataFromCollectionData(c,
this._options.keyField,this._options.valField);$.extend(g,this._options.seriesOptions);$.extend(g,{type:"pie",name:this._options.seriesName,data:c});this._chart.addSeries(g);break;case "line":c=this.lineDataFromCollectionData(this._options.seriesField,this._options.keyField,this._options.valField,this._options.orderBy),this._options.chartOptions.xAxis=c.xAxis,this._options.chartOptions.series=c.series,this._selector.highcharts(this._options.chartOptions),this._chart=this._selector.highcharts()}this._hookEvents()};
e=e.modules.Collection;h=e.prototype.init;a.prototype.pieDataFromCollectionData=function(a,b,c){var g=[],f;for(f=0;f<a.length;f++)g.push([a[f][b],a[f][c]]);return g};a.prototype.lineDataFromCollectionData=function(a,b,c,g){var f=this._collection.distinct(a),h=[],e={categories:[]},k,m,R,Y,A;for(Y=0;Y<f.length;Y++){k=f[Y];m={};m[a]=k;R=[];m=this._collection.find(m,{orderBy:g});for(A=0;A<m.length;A++)e.categories.push(m[A][b]),R.push(m[A][c]);h.push({name:k,data:R})}return{xAxis:e,series:h}};a.prototype._hookEvents=
...
function(a,d){var b=this._data[a],c;c=b.indexOf(d);-1<c&&(b.splice(c,1),this._size--,this.pullFromCrossRef(d,b));b.length||delete this._data[a]};e.prototype.pull=function(a){var d=a[this._collection.primaryKey()],b=this._crossRef[d],c,g=b.length,f;for(c=0;c<g;c++)f=b[c],this._pullFromArray(f,a);this._size--;delete this._crossRef[d]};e.prototype._pullFromArray=function(a,d){for(var b=a.length;b--;)a[b]===d&&a.splice(b,1)};e.prototype.pushToCrossRef=function(a,d){var b=a[this._collection.primaryKey()];
this._crossRef[b]=this._crossRef[b]||[];b=this._crossRef[b];-1===b.indexOf(d)&&b.push(d)};e.prototype.pullFromCrossRef=function(a,d){var b=a[this._collection.primaryKey()];delete this._crossRef[b]};e.prototype.lookup=function(a){return this._data[this._itemHash(a,this._keys)]||[]};e.prototype.match=function(a,d){return(new h).countObjectPaths(this._keys,a)};e.prototype._itemHash=function(a,d){var b=new h,c,g="",f;c=b.parse(d);for(f=0;f<c.length;f++)g&&(g+="_"),g+=b.value(a,c[f].path).join(":");return g};
e.prototype._itemKeyHash=function(a,d){var b=new h,c,g="",f;c=b.parse(d);for(f=0;f<c.length;f++)g&&(g+="_"),g+=b.keyValue(a,c[f].path);return g};e.prototype._itemHashArr=function(a,d){var b=new h,c,g=[],f,e,k,m;c=b.parse(d);for(k=0;k<c.length;k++)for(f=b.value(a,c[k].path),e=0;e<f.length;e++)if(0===k)g.push(f[e]);else for(m=0;m<g.length;m++)g[m]=g[m]+"_"+f[e];return g};m.exports=e},{"./Path":13,"./Shared":15}],9:[function(e,m,k){e=e("./Shared");k=function(h){this.init.apply(this,arguments)};k.prototype.init=
...
12:[function(e,m,k){var h,a,d;k=e("./Shared");var b=function(){this.init.apply(this,arguments)};b.prototype.init=function(c){this._name=c;this._data=new d("__FDB__dc_data_"+this._name);this._collData=new a;this._collections=[]};k.addModule("Overview",b);k.inherit(b.prototype,k.chainSystem);a=e("./Collection");d=e("./Document");e=k.modules.Core;h=k.modules.Core.prototype.init;k.synthesize(b.prototype,"db");k.synthesize(b.prototype,"name");k.synthesize(b.prototype,"query");k.synthesize(b.prototype,
"queryOptions");k.synthesize(b.prototype,"reduce");b.prototype.debug=k.common.debug;b.prototype.objectId=k.common.objectId;b.prototype.from=function(a){void 0!==a&&("string"===typeof a&&(a=this._db.collection(a)),this._addCollection(a));return this};b.prototype.find=function(){return this._collData.find.apply(this._collData,arguments)};b.prototype._addCollection=function(a){-1===this._collections.indexOf(a)&&(this._collections.push(a),a.chain(this),this._refresh());return this};b.prototype._removeCollection=
function(a){-1<this._collections.indexOf(a)&&(this._collections.splice(a,1),a.unChain(this),this._refresh());return this};b.prototype._refresh=function(){if(this._collections&&this._collections[0]){this._collData.primaryKey(this._collections[0].primaryKey());var a=[],d;for(d=0;d<this._collections.length;d++)a=a.concat(this._collections[d].find(this._query,this._queryOptions));this._collData.setData(a)}this._reduce&&(a=this._reduce(),this._data.setData(a))};b.prototype._chainHandler=function(a,d,b,
h){switch(d){case "setData":case "insert":case "update":case "remove":this._refresh()}};b.prototype.link=function(a,d){this._data.link.apply(this._data,arguments);this._refresh()};b.prototype.unlink=function(a,d){this._data.unlink.apply(this._data,arguments);this._refresh()};e.prototype.init=function(){this._overview={};h.apply(this,arguments)};e.prototype.overview=function(a){return a?(this._overview[a]=this._overview[a]||(new b(a)).db(this),this._overview[a]):this._overview};m.exports=b},{"./Collection":2,
"./Document":6,"./Shared":15}],13:[function(e,m,k){e=e("./Shared");k=function(h){this.init.apply(this,arguments)};k.prototype.init=function(h){h&&this.path(h)};e.addModule("Path",k);e.inherit(k.prototype,e.chainSystem);k.prototype.path=function(h){return void 0!==h?(this._path=this.clean(h),this._pathParts=this._path.split("."),this):this._path};k.prototype.hasObjectPaths=function(h,a){var d=!0,b;for(b in h)if(h.hasOwnProperty(b)&&(void 0===a[b]||"object"===typeof h[b]&&(d=this.hasObjectPaths(h[b],
a[b]),!d)))return!1;return d};k.prototype.countKeys=function(h){var a=0,d;for(d in h)h.hasOwnProperty(d)&&void 0!==h[d]&&("object"!==typeof h[d]?a++:a+=this.countKeys(h[d]));return a};k.prototype.countObjectPaths=function(h,a){var d,b={},c=0,g=0,f;for(f in a)a.hasOwnProperty(f)&&("object"===typeof a[f]?(d=this.countObjectPaths(h[f],a[f]),b[f]=d.matchedKeys,g+=d.totalKeyCount,c+=d.matchedKeyCount):(g++,h&&h[f]&&"object"!==typeof h[f]?(b[f]=!0,c++):b[f]=!1));return{matchedKeys:b,matchedKeyCount:c,totalKeyCount:g}};
k.prototype.parse=function(h,a){var d=[],b="",c,g,f;for(g in h)if(h.hasOwnProperty(g))if(b=g,"object"===typeof h[g])if(a)for(c=this.parse(h[g],a),f=0;f<c.length;f++)d.push({path:b+"."+c[f].path,value:c[f].value});else for(c=this.parse(h[g]),f=0;f<c.length;f++)d.push({path:b+"."+c[f].path});else a?d.push({path:b,value:h[g]}):d.push({path:b});return d};k.prototype.parseArr=function(h,a){a=a||{};return this._parseArr(h,"",[],a)};k.prototype._parseArr=function(h,a,d,b){var c,g="";a=a||"";d=d||[];for(c in h)h.hasOwnProperty(c)&&
(!b.ignore||b.ignore&&!b.ignore.test(c))&&(g=a?a+"."+c:c,"object"===typeof h[c]?this._parseArr(h[c],g,d,b):d.push(g));return d};k.prototype.value=function(h,a){if(void 0!==h&&"object"===typeof h){var d,b,c,g,f=[],e;void 0!==a&&(a=this.clean(a),d=a.split("."));d=d||this._pathParts;b=d.length;c=h;for(e=0;e<b;e++){c=c[d[e]];if(g instanceof Array){for(b=0;b<g.length;b++)f=f.concat(this.value(g,b+"."+d[e]));return f}if(!c||"object"!==typeof c)break;g=c}return[c]}return[]};k.prototype.set=function(e,a,
d){if(void 0!==e&&void 0!==a){var b;a=this.clean(a);a=a.split(".");b=a.shift();a.length?(e[b]=e[b]||{},this.set(e[b],a.join("."),d)):e[b]=d}return e};k.prototype.get=function(e,a){return this.value(e,a)[0]};k.prototype.push=function(e,a,d){if(void 0!==e&&void 0!==a){var b;a=this.clean(a);a=a.split(".");b=a.shift();if(a.length)e[b]=e[b]||{},this.set(e[b],a.join("."),d);else if(e[b]=e[b]||[],e[b]instanceof Array)e[b].push(d);else throw"Cannot push to a path whose endpoint is not an array!";}return e};
k.prototype.keyValue=function(e,a){var d,b,c,g,f;void 0!==a&&(a=this.clean(a),d=a.split("."));d=d||this._pathParts;b=d.length;c=e;for(f=0;f<b;f++)if(c=c[d[f]],!c||"object"!==typeof c){g=d[f]+":"+c;break}return g};k.prototype.clean=function(e){"."===e.substr(0,1)&&(e=e.substr(1,e.length-1));return e};m.exports=k},{"./Shared":15}],14:[function(e,m,k){k=e("./Shared");var h,a,d,b;b=function(){this.init.apply(this,arguments)};b.prototype.init=function(a){a.isClient()&&void 0!==Storage&&this.mode("localStorage")};
k.addModule("Persist",b);k.inherit(b.prototype,k.chainSystem);k=k.modules.Core;h=e("./Collection");a=h.prototype.drop;e("./CollectionGroup");d=k.prototype.init;b.prototype.mode=function(a){return void 0!==a?(this._mode=a,this):this._mode};b.prototype.save=function(a,d,b){switch(this.mode()){case "localStorage":d="object"===typeof d?"json::fdb::"+JSON.stringify(d):"raw::fdb::"+d;try{localStorage.setItem(a,d)}catch(e){b&&b(e)}b&&b(!1)}b&&b("No data handler.")};b.prototype.load=function(a,d){var b,e;
switch(this.mode()){case "localStorage":try{b=localStorage.getItem(a)}catch(h){d(h,null)}if(b){b=b.split("::fdb::");switch(b[0]){case "json":e=JSON.parse(b[1]);break;case "raw":e=b[1]}d&&d(!1,e)}}d&&d("No data handler or unrecognised data type.")};b.prototype.drop=function(a,d){switch(this.mode()){case "localStorage":try{localStorage.removeItem(a)}catch(b){d&&d(b)}d&&d(!1)}d&&d("No data handler or unrecognised data type.")};h.prototype.drop=function(c){if(c)if(this._name)if(this._db)this._db.persist.drop(this._name);
else return callback&&callback("Cannot drop a collection's persistent storage when the collection is not attached to a database!"),"Cannot drop a collection's persistent storage when the collection is not attached to a database!";else return callback&&callback("Cannot drop a collection's persistent storage when no name assigned to collection!"),"Cannot drop a collection's persistent storage when no name assigned to collection!";a.apply(this)};h.prototype.save=function(a){if(this._name)if(this._db)this._db.persist.save(this._name,
this._data);else return a&&a("Cannot save a collection that is not attached to a database!"),"Cannot save a collection that is not attached to a database!";else return a&&a("Cannot save a collection with no assigned name!"),"Cannot save a collection with no assigned name!"};h.prototype.load=function(a){var d=this;if(this._name)if(this._db)this._db.persist.load(this._name,function(b,e){if(b)return a&&a(b),b;e&&d.setData(e);a&&a(!1)});else return a&&a("Cannot load a collection that is not attached to a database!"),
"Cannot load a collection that is not attached to a database!";else return a&&a("Cannot load a collection with no assigned name!"),"Cannot load a collection with no assigned name!"};k.prototype.init=function(){this.persist=new b(this);d.apply(this,arguments)};m.exports=b},{"./Collection":2,"./CollectionGroup":3,"./Shared":15}],15:[function(e,m,k){var h=0;e=function(a){if(a){var d,b=a.length;return function(){for(d=0;d<b;d++)if(a[d].length===arguments.length)return a[d].apply(this,arguments);return null}}return function(){}};
e={modules:{},common:{decouple:function(a){return JSON.parse(JSON.stringify(a))},objectId:function(a){var d=Math.pow(10,17);if(a){var b=0,c=a.length,g;for(g=0;g<c;g++)b+=a.charCodeAt(g)*d;a=b.toString(16)}else h++,a=(h+(Math.random()*d+Math.random()*d+Math.random()*d+Math.random()*d)).toString(16);return a},debug:new e([function(){return this._debug&&this._debug.all},function(a){return void 0!==a?"boolean"===typeof a?(this._debug=this._debug||{},this._debug.all=a,this.chainSend("debug",this._debug),
this):this._debug&&this._debug[a]||this._db&&this._db._debug&&this._db._debug[a]||this._debug&&this._debug.all:this._debug&&this._debug.all},function(a,d){return void 0!==a?void 0!==d?(this._debug=this._debug||{},this._debug[a]=d,this.chainSend("debug",this._debug),this):this._debug&&this._debug[d]||this._db&&this._db._debug&&this._db._debug[a]:this._debug&&this._debug.all}]),on:new e([function(a,d){this._listeners=this._listeners||{};this._listeners[a]=this._listeners[a]||{};this._listeners[a]["*"]=
this._listeners[a]["*"]||[];this._listeners[a]["*"].push(d);return this},function(a,d,b){this._listeners=this._listeners||{};this._listeners[a]=this._listeners[a]||{};this._listeners[a][d]=this._listeners[a][d]||[];this._listeners[a][d].push(b);return this}]),off:new e([function(a){this._listeners&&this._listeners[a]&&a in this._listeners&&delete this._listeners[a];return this},function(a,d){var b,c;"string"===typeof d?this._listeners&&this._listeners[a]&&this._listeners[a][d]&&delete this._listeners[a][d]:
a in this._listeners&&(b=this._listeners[a]["*"],c=b.indexOf(d),-1<c&&b.splice(c,1));return this},function(a,d,b){this._listeners&&a in this._listeners&&(a=this._listeners[a][d],b=a.indexOf(b),-1<b&&a.splice(b,1))}]),emit:function(a,d){this._listeners=this._listeners||{};if(a in this._listeners){if(this._listeners[a]["*"]){var b=this._listeners[a]["*"],c=b.length,g;for(g=0;g<c;g++)b[g].apply(this,Array.prototype.slice.call(arguments,1))}if(d instanceof Array&&d[0]&&d[0][this._primaryKey]){var b=this._listeners[a],
f,e,c=d.length;for(g=0;g<c;g++)if(b[d[g][this._primaryKey]])for(f=b[d[g][this._primaryKey]].length,e=0;e<f;e++)b[d[g][this._primaryKey]][e].apply(this,Array.prototype.slice.call(arguments,1))}}return this}},_synth:{},addModule:function(a,d){this.modules[a]=d},inherit:function(a,d){for(var b in d)d.hasOwnProperty(b)&&(a[b]=d[b])},synthesize:function(a,d){this._synth[d]=this._synth[d]||function(a){return void 0!==a?(this["_"+d]=a,this):this["_"+d]};a[d]=this._synth[d]},overload:e,chainSystem:{chain:function(a){this._chain=
this._chain||[];-1===this._chain.indexOf(a)&&this._chain.push(a)},unChain:function(a){this._chain&&(a=this._chain.indexOf(a),-1<a&&this._chain.splice(a,1))},chainSend:function(a,d,b){if(this._chain){var c=this._chain,g=c.length,f;for(f=0;f<g;f++)c[f].chainReceive(this,a,d,b)}},chainReceive:function(a,d,b,c){(!this._chainHandler||this._chainHandler&&!this._chainHandler(a,d,b,c))&&this.chainSend(d,b,c)}}};m.exports=e},{}],16:[function(e,m,k){var h,a,d;k=e("./Shared");var b=function(a,d,b){this.init.apply(this,
arguments)};b.prototype.init=function(a,d,b){this._name=a;this._collections=[];this._groups=[];this._listeners={};this._querySettings={query:d,options:b};this._debug={};this._privateData=new h("__FDB__view_privateData_"+this._name)};k.addModule("View",b);k.inherit(b.prototype,k.chainSystem);h=e("./Collection");CollectionGroup=e("./CollectionGroup");a=h.prototype.init;e=k.modules.Core;d=e.prototype.init;k.synthesize(b.prototype,"name");b.prototype.debug=k.common.debug;b.prototype.insert=function(){this._collectionsRun("insert",
...
console.log('ForerunnerDB.View: Setting up data binding on view "'+this.name()+'" in underlying (internal) view collection "'+b.name()+'" for output target: '+a);return b.link(a,d)};b.prototype.unlink=function(a,d){var b=this.publicData();this.debug()&&console.log('ForerunnerDB.View: Removing data binding on view "'+this.name()+'" in underlying (internal) view collection "'+b.name()+'" for output target: '+a);return b.unlink(a,d)};b.prototype.decouple=k.common.decouple;b.prototype.from=function(a){void 0!==
a&&("string"===typeof a&&(a=this._db.collection(a)),this._addCollection(a));return this};b.prototype._addCollection=function(a){if(-1===this._collections.indexOf(a)){this._collections.push(a);a.chain(this);var d=a.find(this._querySettings.query,this._querySettings.options);this._transformPrimaryKey(a.primaryKey());this._transformInsert(d);this._privateData.primaryKey(a.primaryKey());this._privateData.insert(d)}return this};b.prototype._removeCollection=function(a){-1<this._collections.indexOf(a)&&
(this._collections.splice(a,1),a.unChain(this),this._privateData.remove(a.find(this._querySettings.query,this._querySettings.options)));return this};b.prototype._chainHandler=function(a,d,b,e){var h,k;switch(d){case "setData":this.debug()&&console.log('ForerunnerDB.View: Setting data on view "'+this.name()+'" in underlying (internal) view collection "'+this._privateData.name()+'"');b=this.decouple(b);this._transformSetData(b);this._privateData.setData(b);break;case "insert":this.debug()&&console.log('ForerunnerDB.View: Inserting some data on view "'+
this.name()+'" in underlying (internal) view collection "'+this._privateData.name()+'"');b=this.decouple(b);if(this._querySettings.options&&this._querySettings.options.$orderBy)if(h=[].concat(this._privateData._data),(a=b instanceof Array)?h=h.concat(b):h.push(b),h=this._privateData.sort(this._querySettings.options.$orderBy,h),a)for(b.sort(function(a,c){return h.indexOf(a)-h.indexOf(c)}),a=0;a<b.length;a++)d=h.indexOf(b[a]),this._transformInsert(b,d),this._privateData._insertHandle(b,d);else d=h.indexOf(b),
this._transformInsert(b,d),this._privateData._insertHandle(b,d);else d=e&&e.index?e.index:this._privateData._data.length,this._transformInsert(b,d),this._privateData._insertHandle(b,d);break;case "update":this.debug()&&console.log('ForerunnerDB.View: Updating some data on view "'+this.name()+'" in underlying (internal) view collection "'+this._privateData.name()+'"');b=this._privateData.update(b.query,b.update,b.options);if(this._querySettings.options&&this._querySettings.options.$orderBy)for(h=[].concat(this._privateData._data),
h=this._privateData.sort(this._querySettings.options.$orderBy,h),b.sort(function(a,c){return h.indexOf(a)-h.indexOf(c)}),a=0;a<b.length;a++)e=this._privateData._data.indexOf(b[a]),d=h.indexOf(b[a]),this._privateData._updateSpliceMove(this._privateData._data,e,d);if(this._transformEnabled&&this._transformIn)for(d=this._publicData.primaryKey(),a=0;a<b.length;a++)e={},k=b[a],e[d]=k[d],this._transformUpdate(e,k);break;case "remove":this.debug()&&console.log('ForerunnerDB.View: Removing some data on view "'+
...
function g(a){Ma=(va._dbgMode=a)?"Unavailable (nested view): use #getIndex()":"";wa("dbg",Ia.dbg=na.dbg=a?c:b)}function f(a){this.name=(r.link?"JsViews":"JsRender")+" Error";this.message=a||this.name}function k(a,d){for(var b in d)a[b]=d[b];return a}function m(a){return"function"===typeof a}function ja(a,d,b){if(!S.rTag||a)x=a?a.charAt(0):x,ha=a?a.charAt(1):ha,N=d?d.charAt(0):N,z=d?d.charAt(1):z,O=b||O,a="\\"+x+"(\\"+O+")?\\"+ha,d="\\"+N+"\\"+z,G="(?:(?:(\\w+(?=[\\/\\s\\"+N+"]))|(?:(\\w+)?(:)|(>)|!--((?:[^-]|-(?!-))*)--|(\\*)))\\s*((?:[^\\"+
N+"]|\\"+N+"(?!\\"+z+"))*?)",S.rTag=G+")",G=RegExp(a+G+"(\\/)?|(?:\\/(\\w+)))"+d,"g"),xa=RegExp("<.*>|([^\\\\]|^)[{}]|"+a+".*"+d);return[x,ha,N,z,O]}function Ja(a,b){b||(b=a,a=d);var c,e,f,g;f=this;c=!b||"root"===b;if(a){if(g=f.type===b?f:d,!g)if(c=f.views,f._.useKey)for(e in c){if(g=c[e].get(a,b))break}else for(e=0,f=c.length;!g&&e<f;e++)g=c[e].get(a,b)}else if(c)for(;f.parent.parent;)g=f=f.parent;else for(;f&&!g;)g=f.type===b?f:d,f=f.parent;return g}function R(){var a=this.get("item");return a?
a.index:d}function Y(){return this.index}function A(a){var b,c=this,f=c.linkCtx,g=(c.ctx||{})[a];g===d&&f&&f.ctx&&(g=f.ctx[a]);g===d&&(g=Ia[a]);g&&m(g)&&!g._wrp&&(b=function(){return g.apply(this&&this!==e?this:c,arguments)},b._wrp=1,k(b,g));return b||g}function n(a,b,c){var e,f,g=+c===c&&c,h=b.linkCtx;g&&(c=(g=b.tmpl.bnds[g-1])(b.data,b,I));e=c.args[0];if(a||g){e=h&&h.tag;e||(e={_:{inline:!h,bnd:g},tagName:":",cvt:a,flow:!0,tagCtx:c,_is:"tag"},h&&(h.tag=e,e.linkCtx=h,c.ctx=J(c.ctx,h.view.ctx)),S._lnk(e));
...
(function(e,a,d){function b(a){return F(a)?new g(a):new c(a)}function c(a){this._data=a;return this}function g(a){this._data=a;return this}function f(a,b){a=F(a)?a:[a];var d,c,e=b,g=e,h=a.length,k=[];for(d=0;d<h;d++)c=a[d],P(c)?k=k.concat(f(c.call(b,b),b)):""+c!==c?(b=g=c,g!==e&&k.push(e=g)):(g!==e&&k.push(e=g),k.push(c));return k}function k(a,b){var d,c;for(d in a){c=!0;break}c||delete ra[b]}function m(a,b){if(!a.data||!a.data.off){var c,e,f,g=b.oldValue,h=b.value,k=a.data,l=k.observeAll,p=!k.cb.noArray,
...
e){var f=a._ob,h=b.length;typeof f===la&&(g(f,!0),(h||X&&F(f))&&R(X,[f],b,x,V,!0));f=a._ob=V(a,d);typeof f===la&&(g(f),(h||X&&F(f))&&R(X,[f],b,x,V,[d]));x(c,e)}}function g(a,e,f,h){if(X){var k=w,l=T;w=a;h&&(w=a[h],T+="."+h,da&&(w=b._fltr(h,a,T,da)));w&&(f||F(w))&&c(Aa+".observe"+(x?".obs"+(ha=x._cId=x._cId||ea++):""),d,!0,e);w=k;T=l}}var h,l,p,r,t,n,q,A,x,ha,N,z,O,V,aa,ia,ga,y,M,T,da,ca,Y,ma,X=!1!=this,ja=1,ba=Da,K=Array.apply(0,arguments);h=K.pop();var W=K.shift(),w=y=W,Q=K.length;W+""===W&&X&&(ca=
W,W=w=y=K.shift(),Q--);if(P(h))x=h;else if(h+""===h&&(T=h,M=K.pop(),da=K.pop(),h=K.pop(),Q-=3),!0===h?A=h:h&&(W=h,ja=0),h=K[Q-1],Q&&h===d||P(h))x=K.pop(),Q--;Q&&P(K[Q-1])&&(V=x,x=K.pop(),Q--);ba+=A?x?".obs"+x._cId:"":".obs"+(ha=x._cId=x._cId||ea++);A||(ia=ra[ha]=ra[ha]||{});Y=ca&&ca.match(La)||[""];for(ma=Y.length;ma--;)for(ca=Y[ma],F(y)?g(y,A,!0):A&&0===Q&&y&&c(ba,""),h=ga=0;h<Q;h++)if(n=K[h],""!==n){w=y;if(""+n===n){r=n.split("^");r[1]&&(ga=r[0].split(".").length,n=r.join("."),ga=n.split(".").length-
...
p++);if(p){w=w[t];continue}}if("*"===t){!A&&O&&O.length&&c(ba,"",!1,!0);P(w)?(q=w.depends)&&R(X,[q],x,A||W):c(ba,"");for(l in w)g(w,A,d,l);break}else t&&c(ba+"."+t,r.join("."))}T&&(T+="."+t);t=w[t]}if(P(t)){(q=t.depends)&&R(X,[w],f(q,w),x,V,A||[W]);break}w=t}g(w,A)}ha&&k(ia,ha);return{cbId:ha,bnd:ia}}function Ja(){[].push.call(arguments,!0);return ja.apply(this,arguments)}function R(){var a=[].concat.apply([],arguments);return ja.apply(a.shift(),a)}function Y(a,b,d,c){a+""!==a&&(d=b,b=a,a="");n(a,
...
x||q!==b)u[n?"prop":"attr"](f,b!==d||n?b:null);c._val=b}return p||x}if(f===U&&g&&g.onBeforeLink)g.onBeforeLink()}function f(a,b){var c=this.hlp(Q),d=this.hlp(I);if(!c||!1!==c.call(this,a,b)){if(b){var c=b.index,e=b.items;switch(b.change){case "insert":this.addViews(c,e);break;case "remove":this.removeViews(c,e.length);break;case "move":this.refresh();break;case "refresh":this.refresh()}}d&&d.call(this,a,b)}}function k(b){var c,e=b.data,g=b._.bnd;if(!b._.useKey&&g){if(c=b._.bndArr)a([c[1]]).off(W,
c[0]),b._.bndArr=d;g!==!!g&&g._.inline?e?g._.arrVws[b._.id]=b:delete g._.arrVws[b._.id]:e&&(c=function(a){a.data&&a.data.off||f.apply(b,arguments)},a([e]).on(W,c),b._.bndArr=[c,e])}}function m(a,b,c){var d=a.nodeName.toLowerCase(),e=T.merge[d]||a.contentEditable===va&&{to:U,from:U};return e?b?"input"===d&&a.type===wa?wa:e.to:e.from:b?c?"text":U:""}function ja(b,c,e,f,g,h,k){var l,m,p,n=b.parentElem,q=b._prv,r=b._nxt,t=b._elCnt;q&&q.parentNode!==n&&ha("Missing parentNode");if(k)for(p in m=b.nodes(),
...
C+' ). Use data-link="..."');v&&(C=v,oa.unshift(ya),ya=v.slice(1),oa[0]&&oa[0]===jb[ya]&&ha("Parent of <tr> must be <tbody>"),ka=Wa[ya],(za=Va[ya])&&!pa&&(Ga.unshift(ua),ua=""),pa=za,ua&&za&&(ua+="+"));return b}function t(a,b){var e,g,h,B,sa=[];if(a){"@"===a._tkns.charAt(0)&&(b=L.previousSibling,L.parentNode.removeChild(L),L=d);for(G=a.length;G--;){E=a[G];h=E.ch;if(e=E.path)for(D=e.length-1;g=e.charAt(D--);)"+"===g?"-"===e.charAt(D)?(D--,b=b.previousSibling):b=b.parentNode:b=b.lastChild;if("^"===
h){if(v=Ba[e=E.id]){g=b&&(!L||L.parentNode!==b);if(!L||g)v.parentElem=b;E.elCnt&&g&&(b._dfr=(E.open?"#":"/")+e+h+(b._dfr||""));sa.push([g?null:L,E])}}else if(z=ia[e=E.id])(z.parentElem||(z.parentElem=b||L&&L.parentNode||c,z._.onRender=Ja,z._.onArrayChange=f,k(z)),g=z.parentElem,E.open)?(z._elCnt=E.elCnt,b?b._dfr="#"+e+h+(b._dfr||""):(z._prv||(g._dfr=l(g._dfr,"#"+e+h)),z._prv=L)):(!b||L&&L.parentNode===b?L&&(z._nxt||(g._dfr=l(g._dfr,"/"+e+h)),z._nxt=L):(b._dfr="/"+e+h+(b._dfr||""),z._nxt=d),y=z.linkCtx,
...
L===e;else if(g&&L===g)break;else L.parentNode&&Ha(F(L,d,W&&ob))&&!S&&L.getAttribute(N)&&xa.push([L]);wa===Fa&&h===d&&c.getAttribute(N)&&xa.push([c]);aa(e,za);aa(g,za);if(!S){if(za&&ua+Ea&&(L=g,ua&&(g?t(F(ua+"+",1),g):t(F(ua,1),c)),t(F(Ea,1),c),g)){U=g.getAttribute(ba);if(H=U.indexOf(ra)+1)U=U.slice(H+ra.length-1);g.setAttribute(ba,Ea+U)}H=xa.length;for(A=0;A<H;A++)if(L=xa[A],J=L[1],L=L[0],J){if(v=Ba[J.id]){if(y=v.linkCtx)v=y.tag,v.linkCtx=y;J.open?(L&&(v.parentElem=L.parentNode,v._prv=L),v._elCnt=
...
m.tmpl?Q="/"+m._.id+"_":(S=m.get,m.tag&&(Da=m.tag+"^",m=!0)),m=!0===m);S&&(Ha=u,W=S.tags,X=S.deep,Y=S.name);c=c?""+c===c?a(c)[0]:c.jquery?c[0]:c:wa.parentElem||ga.body;da=!T.noValidate&&c.contentEditable!==va;ya=c.tagName.toLowerCase();za=!!Va[ya];e=e&&la(e,za);g=g&&la(g,za)||null;if(h!=d){na=ea=ga.createElement("div");ra=Ea="";Z="http://www.w3.org/2000/svg"===c.namespaceURI?"svg_ns":(K=hb.exec(h))&&K[1]||"";x&&K&&K[2]&&ha("Unsupported: "+K[2]);if(za){for(P=g;P&&!(R=F(P));)P=P.nextSibling;if(U=R?
R._tkns:c._dfr){K=Q||"";if(m||!Q)K+="#"+Da;D=U.indexOf(K);D+1&&(D+=K.length,ra=Ea=U.slice(0,D),U=U.slice(D),R?P.setAttribute(ba,U):c._dfr=U)}}ka=d;h=(""+h).replace(lb,s);da&&oa.length&&Ta("Mismatched '<"+ya+"...>' in:\n"+h);if(p)return;ab.appendChild(ea);Z=V[Z]||V.div;m=Z[0];for(na.innerHTML=Z[1]+h+Z[2];m--;)na=na.lastChild;ab.removeChild(ea);for(ea=ga.createDocumentFragment();Z=na.firstChild;)ea.appendChild(Z);c.insertBefore(ea,g)}qa?setTimeout(w,0):w();return qa&&qa.promise()}function q(b,c,e,f,
