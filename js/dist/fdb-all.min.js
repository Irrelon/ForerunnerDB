...
else throw"Cannot splicePush with a key that is not an array! ("+m+")!";break;case "$move":if(k[m]instanceof Array)for(q=0;q<k[m].length;q++){if(this._match(k[m][q],a[m])){f=a[m].$index;if(void 0!==f)this._updateSpliceMove(k[m],q,f),f=!0;else throw"Cannot move without a $index integer value!";break}}else throw"Cannot pull from a key that is not an array! ("+m+")!";break;case "$mul":this._updateMultiply(k,m,a[m]);f=!0;break;default:k[m]!==a[m]&&(this._updateProperty(k,m,a[m]),f=!0)}else if(null!==
k[m]&&"object"===typeof k[m])if(p=k[m]instanceof Array,q=a[m]instanceof Array,p||q)if(!q&&p)for(q=0;q<k[m].length;q++)(h=this._updateObject(k[m][q],a[m],c,b,g+"."+m,e))&&(f=!0);else k[m]!==a[m]&&(this._updateProperty(k,m,a[m]),f=!0);else(h=this._updateObject(k[m],a[m],c,b,g+"."+m,e))&&(f=!0);else k[m]!==a[m]&&(this._updateProperty(k,m,a[m]),f=!0)}return f};c.prototype._isPositionalKey=function(k){return".$"===k.substr(k.length-2,2)};c.prototype._updateProperty=function(k,a,c){this._linked?($.observable(k).setProperty(a,
c),this.debug()&&console.log('ForerunnerDB.Collection: Setting data-bound document property "'+a+'" for collection "'+this.name()+'"')):(k[a]=c,this.debug()&&console.log('ForerunnerDB.Collection: Setting non-data-bound document property "'+a+'" for collection "'+this.name()+'"'))};c.prototype._updateIncrement=function(k,a,c){this._linked?$.observable(k).setProperty(a,k[a]+c):k[a]+=c};c.prototype._updateSpliceMove=function(k,a,c){this._linked?($.observable(k).move(a,c),this.debug()&&console.log('ForerunnerDB.Collection: Moving data-bound document array index from "'+
a+'" to "'+c+'" for collection "'+this.name()+'"')):(k.splice(c,0,k.splice(a,1)[0]),this.debug()&&console.log('ForerunnerDB.Collection: Moving non-data-bound document array index from "'+a+'" to "'+c+'" for collection "'+this.name()+'"'))};c.prototype._updateSplicePush=function(k,a,c){k.length>a?this._linked?$.observable(k).insert(a,c):k.splice(a,0,c):this._linked?$.observable(k).insert(c):k.push(c)};c.prototype._updatePush=function(k,a){this._linked?$.observable(k).insert(a):k.push(a)};c.prototype._updatePull=
function(k,a){this._linked?$.observable(k).remove(a):k.splice(a,1)};c.prototype._updateMultiply=function(k,a,c){this._linked?$.observable(k).setProperty(a,k[a]*c):k[a]*=c};c.prototype.remove=function(a){var c,b,d=this._views,g;if(a instanceof Array){d=[];for(c=0;c<a.length;c++)d.push(this.remove(a[c]));return d}c=this.find(a,{decouple:!1});if(c.length){for(g=0;g<c.length;g++)b=c[g],this._removeIndex(b),b=this._data.indexOf(b),this._linked?$.observable(this._data).remove(b):this._data.splice(b,1);
...
return h.find({},b)}return this.find(a,b)};c.prototype.explain=function(a,c){return this.find(a,c).__fdbOp._data};c.prototype.find=function(a,c){a=a||{};c=c||{};c.decouple=void 0!==c.decouple?c.decouple:!0;var b=this._metrics.create("find"),g=this,e,f=!0,h,l,n,p,q,m,u,s,r,x=[];l=function(c){return g._match(c,a,"and")};b.start();if(a){b.time("analyseQuery");e=this._analyseQuery(a,c,b);b.time("analyseQuery");b.data("analysis",e);if(e.hasJoin&&e.queriesJoin){b.time("joinReferences");for(n=0;n<e.joinsOn.length;n++)q=
e.joinsOn[n],p=new d(e.joinQueries[q]),p=p.value(a)[0],this._db.collection(e.joinsOn[n]).subset(p);b.time("joinReferences")}e.indexMatch.length&&(!c||c&&!c.skipIndex)?(b.data("index.potential",e.indexMatch),b.data("index.used",e.indexMatch[0].index),b.time("indexLookup"),h=e.indexMatch[0].lookup,b.time("indexLookup"),e.indexMatch[0].keyData.totalKeyCount===e.indexMatch[0].keyData.matchedKeyCount&&(f=!1)):b.flag("usedIndex",!1);f&&(h&&h.length?(e=h.length,b.time("tableScan: "+e),h=h.filter(l)):(e=
this._data.length,b.time("tableScan: "+e),h=this._data.filter(l)),c.sort&&(b.time("sort"),h=this.sort(c.sort,h),b.time("sort")),b.time("tableScan: "+e));c.limit&&h&&h.length>c.limit&&(h.length=c.limit,b.data("limit",c.limit));c.decouple&&(b.time("decouple"),h=this.decouple(h),b.time("decouple"),b.data("flag.decouple",!0));if(c.join){for(l=0;l<c.join.length;l++)for(q in c.join[l])if(c.join[l].hasOwnProperty(q))for(s=q,e=this._db.collection(q),f=c.join[l][q],r=0;r<h.length;r++){u={};p=n=!1;for(m in f)if(f.hasOwnProperty(m))if("$"===
m.substr(0,1))switch(m){case "$as":s=f[m];break;case "$multi":n=f[m];break;case "$require":p=f[m];break;default:m.substr(0,3)}else u[m]=(new d(f[m])).value(h[r])[0];u=e.find(u);!p||p&&u[0]?h[r][s]=!1===n?u[0]:u:x.push(h[r])}b.data("flag.join",!0)}if(x.length){b.time("removalQueue");for(m=0;m<x.length;m++)q=h.indexOf(x[m]),-1<q&&h.splice(q,1);b.time("removalQueue")}if(c.transform){b.time("transform");for(m=0;m<h.length;m++)h.splice(m,1,c.transform(h[m]));b.time("transform");b.data("flag.transform",
...
a.push(this._data[b]);return a}if(f.$or&&f.$or instanceof Array){a=[];for(b=0;b<f.$or.length;b++)a=a.concat(this.lookup(f.$or[b]));return a}}else return d=this._data[f],void 0!==d?[d]:[]};l.prototype.unSet=function(f){delete this._data[f];return this};l.prototype.uniqueSet=function(f,b){return void 0===this._data[f]?(this._data[f]=b,!0):!1};n.exports=l},{"./Shared":16}],9:[function(h,n,l){l=h("./Shared");var f=h("./Operation");h=function(){this.init.apply(this,arguments)};h.prototype.init=function(){this._data=
...
break;case "remove":this._bindRemove(k,a[k],e,d,c)}};h.prototype._bindChange=function(b){this.debug()&&console.log("ForerunnerDB.OldView.Bind: Bind data change, refreshing bind...",b);this.bindRefresh(b)};h.prototype._bindInsert=function(b,e,d,a,g){var c=$(b),k;for(k=0;k<d.length;k++)b=c.find("#"+d[k][this._primaryKey]),b.length||e.template(d[k],function(a,b,d,k){return function(a){e.insert?e.insert(a,b,d,k):e.prependInsert?c.prepend(a):c.append(a);e.afterInsert&&e.afterInsert(a,b,d,k)}}(b,d[k],a,
...
g,function(a,c,b){return function(){e.remove?e.remove(a,c,b):(a.remove(),e.afterRemove&&e.afterRemove(a,c,b))}}(a,d[c],g)):e.remove?e.remove(a,d[c],g):(a.remove(),e.afterRemove&&e.afterRemove(a,d[c],g)))}},{"./Shared":16}],11:[function(h,n,l){var f,b,e,d,a;f=h("./Shared");var g=function(){this.init.apply(this,arguments)};g.prototype.init=function(a){var b=this;this._name=a;this._groups=[];this._listeners={};this._query={query:{},options:{}};this._onFromSetData=function(){b._onSetData.apply(b,arguments)};
...
console.log("ForerunnerDB.OldView: View has no query or options, getting subset..."),this._data=this._from.subset({}));if(!a&&b)if(this.debug()&&console.log("ForerunnerDB.OldView: Refresh not forced, old data detected..."),e=this._data,b.subsetOf()===e.subsetOf()){this.debug()&&console.log("ForerunnerDB.OldView: Old and new data are from same collection...");g=e.find();a=b.find();f=e._primaryKey;for(p=0;p<g.length;p++)h=g[p],d={},d[f]=h[f],(d=b.find(d)[0])?JSON.stringify(d)!==JSON.stringify(h)&&n.push(h):
l.push(h);for(p=0;p<a.length;p++)h=a[p],d={},d[f]=h[f],e.find(d)[0]||t.push(h);this.debug()&&(console.log("ForerunnerDB.OldView: Removed "+t.length+" rows"),console.log("ForerunnerDB.OldView: Inserted "+l.length+" rows"),console.log("ForerunnerDB.OldView: Updated "+n.length+" rows"));l.length&&this._onInsert(l,[]);n.length&&this._onUpdate(n,[]);t.length&&this._onRemove(t,[])}else this.debug()&&console.log("ForerunnerDB.OldView: Old and new data are from different collections..."),t=b.find(),t.length&&
this._onRemove(t),l=e.find(),l.length&&this._onInsert(l);else this.debug()&&console.log("ForerunnerDB.OldView: Forcing data update",g),this._data=this._from.subset(this._query.query,this._query.options),g=this._data.find(),this.debug()&&console.log("ForerunnerDB.OldView: Emitting change event with data",g),this._onInsert(g,[]);this.debug()&&console.log("ForerunnerDB.OldView: Emitting change");this.emit("change")}return this};g.prototype.count=function(){return this._data&&this._data._data?this._data._data.length:
...
"",[],b)};l.prototype._parseArr=function(f,b,e,d){var a,g="";b=b||"";e=e||[];for(a in f)f.hasOwnProperty(a)&&(!d.ignore||d.ignore&&!d.ignore.test(a))&&(g=b?b+"."+a:a,"object"===typeof f[a]?this._parseArr(f[a],g,e,d):e.push(g));return e};l.prototype.value=function(f,b){if(void 0!==f&&"object"===typeof f){var e,d,a,g,c=[],k;void 0!==b&&(b=this.clean(b),e=b.split("."));e=e||this._pathParts;d=e.length;a=f;for(k=0;k<d;k++){a=a[e[k]];if(g instanceof Array){for(d=0;d<g.length;d++)c=c.concat(this.value(g,
d+"."+e[k]));return c}if(!a||"object"!==typeof a)break;g=a}return[a]}return[]};l.prototype.set=function(f,b,e){if(void 0!==f&&void 0!==b){var d;b=this.clean(b);b=b.split(".");d=b.shift();b.length?(f[d]=f[d]||{},this.set(f[d],b.join("."),e)):f[d]=e}return f};l.prototype.get=function(f,b){return this.value(f,b)[0]};l.prototype.push=function(f,b,e){if(void 0!==f&&void 0!==b){var d;b=this.clean(b);b=b.split(".");d=b.shift();if(b.length)f[d]=f[d]||{},this.set(f[d],b.join("."),e);else if(f[d]=f[d]||[],
