...
a.$index;if(void 0!==d)delete a.$index,this._updateSpliceMove(g[l],p,d),d=!0;else throw"Cannot move without a $index integer value!";break}}else throw"Cannot move on a key that is not an array! ("+l+")";break;case "$mul":this._updateMultiply(g,l,a[l]);d=!0;break;case "$rename":this._updateRename(g,l,a[l]);d=!0;break;case "$unset":this._updateUnset(g,l);d=!0;break;case "$pop":if(g[l]instanceof Array)this._updatePop(g[l],a[l])&&(d=!0);else throw"Cannot pop from a key that is not an array! ("+l+")";
break;default:g[l]!==a[l]&&(this._updateProperty(g,l,a[l]),d=!0)}else if(null!==g[l]&&"object"===typeof g[l])if(p=g[l]instanceof Array,q=a[l]instanceof Array,p||q)if(!q&&p)for(p=0;p<g[l].length;p++)(h=this._updateObject(g[l][p],a[l],e,f,c+"."+l,k))&&(d=!0);else g[l]!==a[l]&&(this._updateProperty(g,l,a[l]),d=!0);else(h=this._updateObject(g[l],a[l],e,f,c+"."+l,k))&&(d=!0);else g[l]!==a[l]&&(this._updateProperty(g,l,a[l]),d=!0)}return d};e.prototype._isPositionalKey=function(g){return".$"===g.substr(g.length-
2,2)};e.prototype._updateProperty=function(g,b,a){this._linked?(jQuery.observable(g).setProperty(b,a),this.debug()&&console.log('ForerunnerDB.Collection: Setting data-bound document property "'+b+'" for collection "'+this.name()+'"')):(g[b]=a,this.debug()&&console.log('ForerunnerDB.Collection: Setting non-data-bound document property "'+b+'" for collection "'+this.name()+'"'))};e.prototype._updateIncrement=function(g,b,a){this._linked?jQuery.observable(g).setProperty(b,g[b]+a):g[b]+=a};e.prototype._updateSpliceMove=
function(g,b,a){this._linked?(jQuery.observable(g).move(b,a),this.debug()&&console.log('ForerunnerDB.Collection: Moving data-bound document array index from "'+b+'" to "'+a+'" for collection "'+this.name()+'"')):(g.splice(a,0,g.splice(b,1)[0]),this.debug()&&console.log('ForerunnerDB.Collection: Moving non-data-bound document array index from "'+b+'" to "'+a+'" for collection "'+this.name()+'"'))};e.prototype._updateSplicePush=function(g,b,a){g.length>b?this._linked?jQuery.observable(g).insert(b,a):
g.splice(b,0,a):this._linked?jQuery.observable(g).insert(a):g.push(a)};e.prototype._updatePush=function(g,b){this._linked?jQuery.observable(g).insert(b):g.push(b)};e.prototype._updatePull=function(g,b){this._linked?jQuery.observable(g).remove(b):g.splice(b,1)};e.prototype._updateMultiply=function(g,b,a){this._linked?jQuery.observable(g).setProperty(b,g[b]*a):g[b]*=a};e.prototype._updateRename=function(g,b,a){var e=g[b];this._linked?(jQuery.observable(g).setProperty(a,e),jQuery.observable(g).removeProperty(b)):
(g[a]=e,delete g[b])};e.prototype._updateUnset=function(g,b){this._linked?jQuery.observable(g).removeProperty(b):delete g[b]};e.prototype._updatePop=function(g,b){var a,e=!1;0<g.length&&(this._linked?(1===b?a=g.length-1:-1===b&&(a=0),-1<a&&(jQuery.observable(arr).remove(a),e=!0)):1===b?(g.pop(),e=!0):-1===b&&(g.shift(),e=!0));return e};e.prototype.remove=function(g,b,a){var e,f;if(g instanceof Array){f=[];for(a=0;a<g.length;a++)f.push(this.remove(g[a],{noEmit:!0}));(!b||b&&!b.noEmit)&&this._onRemove(f);
return f}a=this.find(g,{$decouple:!1});if(a.length){for(f=0;f<a.length;f++)e=a[f],this._removeIndex(e),e=this._data.indexOf(e),this._linked?jQuery.observable(this._data).remove(e):this._data.splice(e,1);this.chainSend("remove",{query:g},b);(!b||b&&!b.noEmit)&&this._onRemove(a);this.deferEmit("change",{type:"remove",data:a})}return a};e.prototype.removeById=function(g){var b={};b[this._primaryKey]=g;return this.remove(b)};e.prototype.deferEmit=function(){var g=this,b;this._noEmitDefer||this._db&&(!this._db||
this._db._noEmitDefer)?this.emit.apply(this,arguments):(b=arguments,this._changeTimeout&&clearTimeout(this._changeTimeout),this._changeTimeout=setTimeout(function(){g.debug()&&console.log("ForerunnerDB.Collection: Emitting "+b[0]);g.emit.apply(g,b)},100))};e.prototype.processQueue=function(g,b){var a=this._deferQueue[g],e=this._deferThreshold[g],f=this._deferTime[g];if(a.length){var c=this;a.length&&(a=a.length>e?a.splice(0,e):a.splice(0,a.length),this[g](a));setTimeout(function(){c.processQueue(g,
b)},f)}else b&&b()};e.prototype.insert=function(g,b,a){"function"===typeof b?(a=b,b=this._data.length):void 0===b&&(b=this._data.length);g=this.transformIn(g);return this._insertHandle(g,b,a)};e.prototype._insertHandle=function(g,b,a){var e=this._deferQueue.insert,f=this._deferThreshold.insert,c=[],k=[];if(g instanceof Array){if(g.length>f){this._deferQueue.insert=e.concat(g);this.processQueue("insert",a);return}for(f=0;f<g.length;f++)e=this._insert(g[f],b+f),!0===e?c.push(g[f]):k.push({doc:g[f],
reason:e})}else e=this._insert(g,b),!0===e?c.push(g):k.push({doc:g,reason:e});this.chainSend("insert",g,{index:b});this._onInsert(c,k);a&&a();this.deferEmit("change",{type:"insert",data:c});return{inserted:c,failed:k}};e.prototype._insert=function(g,b){if(g){var a;this._ensurePrimaryKey(g);if(a=this.insertIndexViolation(g))return"Index violation in index: "+a;this._insertIndex(g);b>this._data.length&&(b=this._data.length);this._linked?jQuery.observable(this._data).insert(b,g):this._data.splice(b,
0,g);return!0}return"No document passed to insert"};e.prototype._insertIndex=function(g){var b=this._indexByName,a,e=JSON.stringify(g);this._primaryIndex.uniqueSet(g[this._primaryKey],g);this._primaryCrc.uniqueSet(g[this._primaryKey],e);this._crcLookup.uniqueSet(e,g);for(a in b)b.hasOwnProperty(a)&&b[a].insert(g)};e.prototype._removeIndex=function(g){var b=this._indexByName,a,e=JSON.stringify(g);this._primaryIndex.unSet(g[this._primaryKey]);this._primaryCrc.unSet(g[this._primaryKey]);this._crcLookup.unSet(e);
for(a in b)b.hasOwnProperty(a)&&b[a].remove(g)};e.prototype.subset=function(g,b){var a=this.find(g,b);return(new e)._subsetOf(this).primaryKey(this._primaryKey).setData(a)};e.prototype.subsetOf=function(){return this.__subsetOf};e.prototype._subsetOf=function(g){this.__subsetOf=g;return this};e.prototype.distinct=function(g,a,e){a=this.find(a,e);g=new b(g);e={};var f=[],c,k;for(k=0;k<a.length;k++)(c=g.value(a[k])[0])&&!e[c]&&(e[c]=!0,f.push(c));return f};e.prototype.decouple=d.common.decouple;e.prototype.findById=
function(g,b){var a={};a[this._primaryKey]=g;return this.find(a,b)[0]};e.prototype.peek=function(g,b){var a=this._data,f=a.length,c,k,d=new e;if("string"===typeof g){for(c=0;c<f;c++)k=JSON.stringify(a[c]),-1<k.indexOf(g)&&d.insert(a[c]);return d.find({},b)}return this.find(g,b)};e.prototype.explain=function(b,a){return this.find(b,a).__fdbOp._data};e.prototype.find=function(g,a){g=g||{};a=a||{};a.$decouple=void 0!==a.$decouple?a.$decouple:!0;var e=this._metrics.create("find"),f=this,c,k=!0,d,h,n,
q,p,l,m,E,ba,ma=[];h=function(a){return f._match(a,g,"and")};e.start();if(g){e.time("analyseQuery");c=this._analyseQuery(g,a,e);e.time("analyseQuery");e.data("analysis",c);if(c.hasJoin&&c.queriesJoin){e.time("joinReferences");for(n=0;n<c.joinsOn.length;n++)p=c.joinsOn[n],q=new b(c.joinQueries[p]),q=q.value(g)[0],this._db.collection(c.joinsOn[n]).subset(q);e.time("joinReferences")}c.indexMatch.length&&(!a||a&&!a.$skipIndex)?(e.data("index.potential",c.indexMatch),e.data("index.used",c.indexMatch[0].index),
...
d=this.decouple(d),e.time("decouple"),e.data("flag.decouple",!0));if(a.join){for(h=0;h<a.join.length;h++)for(p in a.join[h])if(a.join[h].hasOwnProperty(p))for(E=p,c=this._db.collection(p),k=a.join[h][p],ba=0;ba<d.length;ba++){m={};q=n=!1;for(l in k)if(k.hasOwnProperty(l))if("$"===l.substr(0,1))switch(l){case "$as":E=k[l];break;case "$multi":n=k[l];break;case "$require":q=k[l];break;default:l.substr(0,3)}else m[l]=(new b(k[l])).value(d[ba])[0];m=c.find(m);!q||q&&m[0]?d[ba][E]=!1===n?m[0]:m:ma.push(d[ba])}e.data("flag.join",
!0)}if(ma.length){e.time("removalQueue");for(l=0;l<ma.length;l++)p=d.indexOf(ma[l]),-1<p&&d.splice(p,1);e.time("removalQueue")}if(a.transform){e.time("transform");for(l=0;l<d.length;l++)d.splice(l,1,a.transform(d[l]));e.time("transform");e.data("flag.transform",!0)}this._transformEnabled&&this._transformOut&&(e.time("transformOut"),d=this.transformOut(d),e.time("transformOut"));e.data("results",d.length);e.stop()}else e.stop(),d=[];d.__fdbOp=e;return d};e.prototype.indexOf=function(a){if(a=this.find(a,
...
Array){var b=[],e;for(e=0;e<a.length;e++)b[e]=this._transformIn(a[e]);return b}return this._transformIn(a)}return a};e.prototype.transformOut=function(a){if(this._transformEnabled&&this._transformOut){if(a instanceof Array){var b=[],e;for(e=0;e<a.length;e++)b[e]=this._transformOut(a[e]);return b}return this._transformOut(a)}return a};e.prototype.sort=function(a,b){b=b||[];var e=[],f,c;for(f in a)a.hasOwnProperty(f)&&(c={},c[f]=a[f],c.___fdbKey=f,e.push(c));return 2>e.length?this._sort(a,b):this._bucketSort(e,
b)};e.prototype._bucketSort=function(a,b){var e=a.shift(),f,c,k=[];if(0<a.length){b=this._sort(e,b);f=this.bucket(e.___fdbKey,b);for(c in f)f.hasOwnProperty(c)&&(e=[].concat(a),k=k.concat(this._bucketSort(e,f[c])));return k}return this._sort(e,b)};e.prototype._sort=function(a,e){var f=new b,c=f.parse(a,!0)[0];f.path(c.path);return e.sort(1===c.value?function(a,b){var g=f.value(a)[0],e=f.value(b)[0];return"string"===typeof g&&"string"===typeof e?g.localeCompare(e):g>e?1:g<e?-1:0}:function(a,b){var g=
f.value(a)[0],e=f.value(b)[0];return"string"===typeof g&&"string"===typeof e?1===g.localeCompare(e)?-1:1:g>e?-1:g<e?1:0})};e.prototype.bucket=function(a,b){var e,f={};for(e=0;e<b.length;e++)f[b[e][a]]=f[b[e][a]]||[],f[b[e][a]].push(b[e]);return f};e.prototype._analyseQuery=function(a,e,f){var c={queriesOn:[this._name],indexMatch:[],hasJoin:!1,queriesJoin:!1,joinQueries:{},query:a,options:e},k,d=[],h=[],m,n,q,p,l;f.time("checkIndexes");void 0!==a[this._primaryKey]&&(f.time("checkIndexMatch: Primary Key"),
m=new b,c.indexMatch.push({lookup:this._primaryIndex.lookup(a,e),keyData:{matchedKeys:[this._primaryKey],matchedKeyCount:1,totalKeyCount:m.countKeys(a)},index:this._primaryIndex}),f.time("checkIndexMatch: Primary Key"));for(l in this._indexById)if(this._indexById.hasOwnProperty(l)&&(n=this._indexById[l],q=n.name(),f.time("checkIndexMatch: "+q),m=n.match(a,e),p=n.lookup(a,e),0<m.matchedKeyCount&&c.indexMatch.push({lookup:p,keyData:m,index:n}),f.time("checkIndexMatch: "+q),m.totalKeyCount===m.matchedKeyCount))break;
f.time("checkIndexes");1<c.indexMatch.length&&(f.time("findOptimalIndex"),c.indexMatch.sort(function(a,b){return a.keyData.totalKeyCount===a.keyData.matchedKeyCount?-1:b.keyData.totalKeyCount===b.keyData.matchedKeyCount?1:a.keyData.matchedKeyCount===b.keyData.matchedKeyCount?a.lookup.length-b.lookup.length:b.keyData.matchedKeyCount-a.keyData.matchedKeyCount}),f.time("findOptimalIndex"));if(e.join){c.hasJoin=!0;for(f=0;f<e.join.length;f++)for(k in e.join[f])e.join[f].hasOwnProperty(k)&&(d.push(k),
"$as"in e.join[f][k]?h.push(e.join[f][k].$as):h.push(k));for(k=0;k<h.length;k++)if(e=this._queryReferencesCollection(a,h[k],""))c.joinQueries[d[k]]=e,c.queriesJoin=!0;c.joinsOn=d;c.queriesOn=c.queriesOn.concat(d)}return c};e.prototype._queryReferencesCollection=function(a,b,e){for(var f in a)if(a.hasOwnProperty(f)){if(f===b)return e&&(e+="."),e+f;if("object"===typeof a[f])return e&&(e+="."),e+=f,this._queryReferencesCollection(a[f],b,e)}return!1};e.prototype._match=function(a,b,e){var f,c;f=typeof a;
c=typeof b;var k=!0,d;if(!("string"!==f&&"number"!==f||"string"!==c&&"number"!==c))a!==b&&(k=!1);else for(d in b)if(b.hasOwnProperty(d)){f=!1;if("$"===d.substr(0,1))switch(d){case "$gt":if(a>b[d]){if("or"===e)return!0}else k=!1;f=!0;break;case "$gte":if(a>=b[d]){if("or"===e)return!0}else k=!1;f=!0;break;case "$lt":if(a<b[d]){if("or"===e)return!0}else k=!1;f=!0;break;case "$lte":if(a<=b[d]){if("or"===e)return!0}else k=!1;f=!0;break;case "$exists":if(void 0===a!==b[d]){if("or"===e)return!0}else k=!1;
f=!0;break;case "$or":f=!0;for(c=0;c<b[d].length;c++){if(this._match(a,b[d][c],"and"))return!0;k=!1}break;case "$and":f=!0;for(c=0;c<b[d].length;c++)if(!this._match(a,b[d][c],"and"))return!1;break;case "$in":if(b[d]instanceof Array){f=b[d];c=f.length;var h,m=!1;for(h=0;h<c;h++)if(f[h]===a){m=!0;break}if(m){if("or"===e)return!0}else k=!1}else throw"Cannot use a $nin operator on a non-array key: "+d;f=!0;break;case "$nin":if(b[d]instanceof Array){f=b[d];c=f.length;m=!0;for(h=0;h<c;h++)if(f[h]===a){m=
!1;break}if(m){if("or"===e)return!0}else k=!1}else throw"Cannot use a $nin operator on a non-array key: "+d;f=!0;break;case "$ne":if(a!=b[d]){if("or"===e)return!0}else k=!1;f=!0}if(!f&&b[d]instanceof RegExp)if(f=!0,"object"===typeof a&&void 0!==a[d]&&b[d].test(a[d])){if("or"===e)return!0}else k=!1;if(!f)if("object"===typeof b[d])if(void 0!==a[d]){if(a[d]instanceof Array&&!(b[d]instanceof Array))for(f=!1,c=0;c<a[d].length&&!(f=this._match(a[d][c],b[d],void 0));c++);else if(!(a[d]instanceof Array)&&
b[d]instanceof Array)for(f=!1,c=0;c<b[d].length&&!(f=this._match(a[d],b[d][c],void 0));c++);else f="object"===typeof a?this._match(a[d],b[d],void 0):this._match(void 0,b[d],void 0);if(f){if("or"===e)return!0}else k=!1}else if(b[d]&&void 0!==b[d].$exists)if(f=this._match(void 0,b[d],void 0)){if("or"===e)return!0}else k=!1;else k=!1;else if(a&&a[d]===b[d]){if("or"===e)return!0}else if(a&&a[d]&&a[d]instanceof Array&&b[d]&&"object"!==typeof b[d]){f=!1;for(c=0;c<a[d].length&&!(f=this._match(a[d][c],b[d],
void 0));c++);if(f){if("or"===e)return!0}else k=!1}else k=!1;if("and"===e&&!k)return!1}return k};e.prototype.count=function(a,b){return a?this.find(a,b).length:this._data.length};e.prototype.link=function(a,b){if(window.jQuery){this._links=this._links||{};var e,f;b&&"object"===typeof b?b.template&&"string"===typeof b.template&&(e=this.objectId(b.template),f=b.template):e=b;if(!this._links[e]){if(jQuery(a).length){if(!jQuery.templates[e]){if(!f)if(f=jQuery(b),f.length)f=jQuery(f[0]).html();else throw"Unable to bind collection to target because template does not exist: "+
b;jQuery.views.templates(e,f)}jQuery.templates[e].link(a,this._data);this._links[e]=a;this._linked++;this.debug()&&console.log('ForerunnerDB.Collection: Added binding collection "'+this.name()+'" to output target: '+a);return this}throw'Cannot bind view data to output target selector "'+a+'" because it does not exist in the DOM!';}throw"Cannot create a duplicate link to the target: "+a+" with the template: "+e;}throw"Cannot data-bind without jQuery, please add jQuery to your page!";};e.prototype.unlink=
...
return a};m.exports=e},{"./Crc":6,"./Index":9,"./KeyValueStore":10,"./Metrics":11,"./Path":14,"./Shared":17}],4:[function(h,m,d){var k,c;d=h("./Shared");var b=function(){this.init.apply(this,arguments)};b.prototype.init=function(a){this._name=a;this._data=new c("__FDB__cg_data_"+this._name);this._collections=[];this._views=[]};d.addModule("CollectionGroup",b);d.inherit(b.prototype,d.chainReactor);c=h("./Collection");h=d.modules.Core;k=d.modules.Core.prototype.init;b.prototype.debug=d.common.debug;
...
arguments)};b.prototype.remove=function(){this._collectionsRun("remove",arguments)};b.prototype._collectionsRun=function(a,b){for(var e=0;e<this._collections.length;e++)this._collections[e][a].apply(this._collections[e],b)};b.prototype.find=function(a,b){return this._data.find(a,b)};b.prototype.removeById=function(a){for(var b=0;b<this._collections.length;b++)this._collections[b].removeById(a)};b.prototype.subset=function(a,b){var e=this.find(a,b);return(new c)._subsetOf(this).primaryKey(this._primaryKey).setData(e)};
b.prototype.drop=function(){var a,b=[].concat(this._collections),e=[].concat(this._views);this._debug&&console.log("Dropping collection group "+this._name);for(a=0;a<b.length;a++)this.removeCollection(b[a]);for(a=0;a<e.length;a++)this._removeView(e[a]);this.emit("drop");return!0};h.prototype.init=function(){this._collectionGroup={};k.apply(this,arguments)};h.prototype.collectionGroup=function(a){return a?(this._collectionGroup[a]=this._collectionGroup[a]||(new b(a)).db(this),this._collectionGroup[a]):
this._collectionGroup};m.exports=b},{"./Collection":3,"./Shared":17}],5:[function(h,m,d){var k,c;k=h("./Shared.js");d=function(){this.init.apply(this,arguments)};d.prototype.init=function(){this._collection={};this._debug={};this._version="1.2.7"};d.prototype.moduleLoaded=k.overload({string:function(b){if(void 0!==b){b=b.replace(/ /g,"");var a=b.split(","),f;for(f=0;f<a.length;f++)if(!k.modules[b])return!1;return!0}return!1},"string, function":function(b,a){if(void 0!==b){b=b.replace(/ /g,"");var f=
b.split(","),e;for(e=0;e<f.length;e++)if(!k.modules[b])return!1;a()}},"string, function, function":function(b,a,f){if(void 0!==b){b=b.replace(/ /g,"");var e=b.split(","),c;for(c=0;c<e.length;c++)if(!k.modules[b])return f(),!1;a()}}});d.moduleLoaded=d.prototype.moduleLoaded;d.shared=k;d.prototype.shared=k;k.addModule("Core",d);k.inherit(d.prototype,k.chainReactor);c=h("./Collection.js");h("./Metrics.js");h=h("./Crc.js");d.prototype._isServer=!1;d.prototype.isClient=function(){return!this._isServer};
d.prototype.isServer=function(){return this._isServer};d.prototype.crc=h;d.prototype.isClient=function(){return!this._isServer};d.prototype.isServer=function(){return this._isServer};d.prototype.decouple=k.common.decouple;d.prototype.debug=k.common.debug;d.prototype.arrayToCollection=function(b){return(new c).setData(b)};d.prototype.on=function(b,a){this._listeners=this._listeners||{};this._listeners[b]=this._listeners[b]||[];this._listeners[b].push(a);return this};d.prototype.off=function(b,a){if(b in
this._listeners){var f=this._listeners[b],e=f.indexOf(a);-1<e&&f.splice(e,1)}return this};d.prototype.emit=function(b,a){this._listeners=this._listeners||{};if(b in this._listeners){var f=this._listeners[b],e=f.length,c;for(c=0;c<e;c++)f[c].apply(this,Array.prototype.slice.call(arguments,1))}return this};d.prototype.objectId=k.common.objectId;d.prototype.peek=function(b){var a,f,e=[],c=typeof b;for(a in this._collection)this._collection.hasOwnProperty(a)&&(f=this._collection[a],e="string"===c?e.concat(f.peek(b)):
e.concat(f.find(b)));return e};d.prototype.peekCat=function(b){var a,f,e={},c,d=typeof b;for(a in this._collection)this._collection.hasOwnProperty(a)&&(f=this._collection[a],(c="string"===d?f.peek(b):f.find(b))&&c.length&&(e[f.name()]=c));return e};m.exports=d},{"./Collection.js":3,"./Crc.js":6,"./Metrics.js":11,"./Shared.js":17}],6:[function(h,m,d){var k=function(){var c=[],b,a,f;for(a=0;256>a;a++){b=a;for(f=0;8>f;f++)b=b&1?3988292384^b>>>1:b>>>1;c[a]=b}return c}();m.exports=function(c){var b=-1,
a;for(a=0;a<c.length;a++)b=b>>>8^k[(b^c.charCodeAt(a))&255];return(b^-1)>>>0}},{}],7:[function(h,m,d){var k,c;d=h("./Shared");var b=function(){this.init.apply(this,arguments)};b.prototype.init=function(a){this._name=a;this._data={}};d.addModule("Document",b);d.inherit(b.prototype,d.chainReactor);h=h("./Collection");k=d.modules.Core;c=d.modules.Core.prototype.init;d.synthesize(b.prototype,"db");d.synthesize(b.prototype,"name");b.prototype.setData=function(a){var b,e;if(a)if(a=this.decouple(a),this._linked){e=
{};for(b in this._data)"jQuery"!==b.substr(0,6)&&this._data.hasOwnProperty(b)&&void 0===a[b]&&(e[b]=1);a.$unset=e;this._updateObject(this._data,a,{})}else this._data=a;return this};b.prototype.decouple=d.common.decouple;b.prototype.objectId=d.common.objectId;b.prototype.on=d.common.on;b.prototype.off=d.common.off;b.prototype.emit=d.common.emit;b.prototype.debug=d.common.debug;b.prototype.update=function(a,b,e){this._updateObject(this._data,b,a,e)};b.prototype._updateObject=h.prototype._updateObject;
b.prototype._isPositionalKey=function(a){return".$"===a.substr(a.length-2,2)};b.prototype._updateProperty=function(a,b,e){this._linked?(jQuery.observable(a).setProperty(b,e),this.debug()&&console.log('ForerunnerDB.Document: Setting data-bound document property "'+b+'" for collection "'+this.name()+'"')):(a[b]=e,this.debug()&&console.log('ForerunnerDB.Document: Setting non-data-bound document property "'+b+'" for collection "'+this.name()+'"'))};b.prototype._updateIncrement=function(a,b,e){this._linked?
jQuery.observable(a).setProperty(b,a[b]+e):a[b]+=e};b.prototype._updateSpliceMove=function(b,f,e){this._linked?(jQuery.observable(b).move(f,e),this.debug()&&console.log('ForerunnerDB.Document: Moving data-bound document array index from "'+f+'" to "'+e+'" for collection "'+this.name()+'"')):(b.splice(e,0,b.splice(f,1)[0]),this.debug()&&console.log('ForerunnerDB.Document: Moving non-data-bound document array index from "'+f+'" to "'+e+'" for collection "'+this.name()+'"'))};b.prototype._updateSplicePush=
function(b,f,e){b.length>f?this._linked?jQuery.observable(b).insert(f,e):b.splice(f,0,e):this._linked?jQuery.observable(b).insert(e):b.push(e)};b.prototype._updatePush=function(b,f){this._linked?jQuery.observable(b).insert(f):b.push(f)};b.prototype._updatePull=function(b,f){this._linked?jQuery.observable(b).remove(f):b.splice(f,1)};b.prototype._updateMultiply=function(b,f,e){this._linked?jQuery.observable(b).setProperty(f,b[f]*e):b[f]*=e};b.prototype._updateRename=function(b,f,e){var c=b[f];this._linked?
(jQuery.observable(b).setProperty(e,c),jQuery.observable(b).removeProperty(f)):(b[e]=c,delete b[f])};b.prototype._updateUnset=function(b,f){this._linked?jQuery.observable(b).removeProperty(f):delete b[f]};b.prototype._updatePop=function(b,f){var e,c=!1;0<b.length&&(this._linked?(1===f?e=b.length-1:-1===f&&(e=0),-1<e&&(jQuery.observable(arr).remove(e),c=!0)):1===f?(b.pop(),c=!0):-1===f&&(b.shift(),c=!0));return c};b.prototype.link=function(b,f){if(window.jQuery){this._links=this._links||{};this._linked||
(this._linked=0);var e,c;f&&"object"===typeof f?f.template&&"string"===typeof f.template&&(e=this.objectId(f.template),c=f.template):e=f;if(!this._links[e]){if(jQuery(b).length){if(!jQuery.templates[e]){if(!c)if(c=jQuery(f),c.length)c=jQuery(c[0]).html();else throw"Unable to bind document to target because template does not exist: "+f;jQuery.views.templates(e,c)}jQuery.templates[e].link(b,this._data);this._links[e]=b;this._linked++;this.debug()&&console.log('ForerunnerDB.Document: Added binding document "'+
...
this._queryOptions));this._collData.setData(b)}this._reduce&&(b=this._reduce(),this._data.setData(b))};a.prototype._chainHandler=function(b){switch(b.type){case "setData":case "insert":case "update":case "remove":this._refresh()}};a.prototype.link=function(b,a){this._data.link.apply(this._data,arguments);this._refresh()};a.prototype.unlink=function(b,a){this._data.unlink.apply(this._data,arguments);this._refresh()};h.prototype.init=function(){this._overview={};k.apply(this,arguments)};h.prototype.overview=
function(b){return b?(this._overview[b]=this._overview[b]||(new a(b)).db(this),this._overview[b]):this._overview};m.exports=a},{"./Collection":3,"./Document":7,"./Shared":17}],14:[function(h,m,d){h=h("./Shared");d=function(d){this.init.apply(this,arguments)};d.prototype.init=function(d){d&&this.path(d)};h.addModule("Path",d);h.inherit(d.prototype,h.chainReactor);d.prototype.path=function(d){return void 0!==d?(this._path=this.clean(d),this._pathParts=this._path.split("."),this):this._path};d.prototype.hasObjectPaths=
function(d,c){var b=!0,a;for(a in d)if(d.hasOwnProperty(a)&&(void 0===c[a]||"object"===typeof d[a]&&(b=this.hasObjectPaths(d[a],c[a]),!b)))return!1;return b};d.prototype.countKeys=function(d){var c=0,b;for(b in d)d.hasOwnProperty(b)&&void 0!==d[b]&&("object"!==typeof d[b]?c++:c+=this.countKeys(d[b]));return c};d.prototype.countObjectPaths=function(d,c){var b,a={},f=0,e=0,g;for(g in c)c.hasOwnProperty(g)&&("object"===typeof c[g]?(b=this.countObjectPaths(d[g],c[g]),a[g]=b.matchedKeys,e+=b.totalKeyCount,
f+=b.matchedKeyCount):(e++,d&&d[g]&&"object"!==typeof d[g]?(a[g]=!0,f++):a[g]=!1));return{matchedKeys:a,matchedKeyCount:f,totalKeyCount:e}};d.prototype.parse=function(d,c){var b=[],a="",f,e,g;for(e in d)if(d.hasOwnProperty(e))if(a=e,"object"===typeof d[e])if(c)for(f=this.parse(d[e],c),g=0;g<f.length;g++)b.push({path:a+"."+f[g].path,value:f[g].value});else for(f=this.parse(d[e]),g=0;g<f.length;g++)b.push({path:a+"."+f[g].path});else c?b.push({path:a,value:d[e]}):b.push({path:a});return b};d.prototype.parseArr=
function(d,c){c=c||{};return this._parseArr(d,"",[],c)};d.prototype._parseArr=function(d,c,b,a){var f,e="";c=c||"";b=b||[];for(f in d)d.hasOwnProperty(f)&&(!a.ignore||a.ignore&&!a.ignore.test(f))&&(e=c?c+"."+f:f,"object"===typeof d[f]?this._parseArr(d[f],e,b,a):b.push(e));return b};d.prototype.value=function(d,c){if(void 0!==d&&"object"===typeof d){var b,a,f,e,g=[],h;void 0!==c&&(c=this.clean(c),b=c.split("."));b=b||this._pathParts;a=b.length;f=d;for(h=0;h<a;h++){f=f[b[h]];if(e instanceof Array){for(a=
0;a<e.length;a++)g=g.concat(this.value(e,a+"."+b[h]));return g}if(!f||"object"!==typeof f)break;e=f}return[f]}return[]};d.prototype.set=function(d,c,b){if(void 0!==d&&void 0!==c){var a;c=this.clean(c);c=c.split(".");a=c.shift();c.length?(d[a]=d[a]||{},this.set(d[a],c.join("."),b)):d[a]=b}return d};d.prototype.get=function(d,c){return this.value(d,c)[0]};d.prototype.push=function(d,c,b){if(void 0!==d&&void 0!==c){var a;c=this.clean(c);c=c.split(".");a=c.shift();if(c.length)d[a]=d[a]||{},this.set(d[a],
c.join("."),b);else if(d[a]=d[a]||[],d[a]instanceof Array)d[a].push(b);else throw"Cannot push to a path whose endpoint is not an array!";}return d};d.prototype.keyValue=function(d,c){var b,a,f,e,g;void 0!==c&&(c=this.clean(c),b=c.split("."));b=b||this._pathParts;a=b.length;f=d;for(g=0;g<a;g++)if(f=f[b[g]],!f||"object"!==typeof f){e=b[g]+":"+f;break}return e};d.prototype.clean=function(d){"."===d.substr(0,1)&&(d=d.substr(1,d.length-1));return d};m.exports=d},{"./Shared":17}],15:[function(h,m,d){d=
h("./Shared");var k,c,b,a;a=function(){this.init.apply(this,arguments)};a.prototype.init=function(b){b.isClient()&&void 0!==Storage&&this.mode("localStorage")};d.addModule("Persist",a);d.inherit(a.prototype,d.chainReactor);d=d.modules.Core;k=h("./Collection");c=k.prototype.drop;h("./CollectionGroup");b=d.prototype.init;a.prototype.mode=function(b){return void 0!==b?(this._mode=b,this):this._mode};a.prototype.save=function(b,a,c){switch(this.mode()){case "localStorage":a="object"===typeof a?"json::fdb::"+
JSON.stringify(a):"raw::fdb::"+a;try{localStorage.setItem(b,a)}catch(d){if(c)c(d);else throw d;break}if(c)c(!1);else return!1;break;default:if(c)c("No data handler.");else throw"No data handler.";}};a.prototype.load=function(b,a){var c,d;switch(this.mode()){case "localStorage":try{c=localStorage.getItem(b)}catch(h){a(h,null)}if(c){c=c.split("::fdb::");switch(c[0]){case "json":d=JSON.parse(c[1]);break;case "raw":d=c[1]}if(a)a(!1,d);else return d}break;default:if(a)a("No data handler or unrecognised data type.");
else throw"No data handler or unrecognised data type.";}};a.prototype.drop=function(b,a){switch(this.mode()){case "localStorage":try{localStorage.removeItem(b)}catch(c){a&&a(c)}a&&a(!1);break;default:a&&a("No data handler or unrecognised data type.")}};k.prototype.drop=function(b){if(b)if(this._name)if(this._db)this._db.persist.drop(this._name);else return callback&&callback("Cannot drop a collection's persistent storage when the collection is not attached to a database!"),"Cannot drop a collection's persistent storage when the collection is not attached to a database!";
else return callback&&callback("Cannot drop a collection's persistent storage when no name assigned to collection!"),"Cannot drop a collection's persistent storage when no name assigned to collection!";c.apply(this)};k.prototype.save=function(b){if(this._name)if(this._db)this._db.persist.save(this._name,this._data,b);else return b&&b("Cannot save a collection that is not attached to a database!"),"Cannot save a collection that is not attached to a database!";else return b&&b("Cannot save a collection with no assigned name!"),
...
new a(this);b.apply(this,arguments)};m.exports=a},{"./Collection":3,"./CollectionGroup":4,"./Shared":17}],16:[function(h,m,d){h=h("./Shared");d=function(d,c,b){if(d&&c&&b){this._reactorIn=d;this._reactorOut=c;this._chainHandler=b;if(!d.chain||!c.chainReceive)throw"ReactorIO requires passed in and out objects to implement the ChainReactor mixin!";d.chain(this);this.chain(c)}else throw"ReactorIO requires an in, out and process argument to instantiate!";};d.prototype.drop=function(){this._reactorIn.unChain(this);
this.unChain(this._reactorOut);delete this._reactorIn;delete this._reactorOut;delete this._chainHandler};h.addModule("ReactorIO",d);h.inherit(d.prototype,h.chainReactor);m.exports=d},{"./Shared":17}],17:[function(h,m,d){var k=0,c=function(b){var a=[],f,e=["string","object","number","function","undefined"],d;if(-1<b.indexOf("*"))for(d=0;d<e.length;d++)f=b.replace("*",e[d]),a=a.concat(c(f));else a.push(b);return a};d=function(b){if(b){var a,f,e,d,h;if(!(b instanceof Array)){e={};for(a in b)if(b.hasOwnProperty(a))if(d=
a.replace(/ /g,""),-1===d.indexOf("*"))e[d]=b[a];else for(h=c(d),d=0;d<h.length;d++)e[h[d]]||(e[h[d]]=b[a]);b=e}return function(){if(b instanceof Array)for(f=b.length,a=0;a<f;a++){if(b[a].length===arguments.length)return b[a].apply(this,arguments)}else{var e=[],c;for(a=0;a<arguments.length;a++)e.push(typeof arguments[a]);c=e.join(",");if(b[c])return b[c].apply(this,arguments);for(a=e.length;0<=a;a--)if(c=e.slice(0,a).join(","),b[c+",..."])return b[c+",..."].apply(this,arguments)}throw"Overloaded method does not have a matching signature for the passed arguments!";
}}return function(){}};h={modules:{},common:{decouple:function(b){if(void 0!==b)return JSON.parse(JSON.stringify(b))},objectId:function(b){var a=Math.pow(10,17);if(b){var c=0,e=b.length,d;for(d=0;d<e;d++)c+=b.charCodeAt(d)*a;b=c.toString(16)}else k++,b=(k+(Math.random()*a+Math.random()*a+Math.random()*a+Math.random()*a)).toString(16);return b},debug:new d([function(){return this._debug&&this._debug.all},function(b){return void 0!==b?"boolean"===typeof b?(this._debug=this._debug||{},this._debug.all=
b,this.chainSend("debug",this._debug),this):this._debug&&this._debug[b]||this._db&&this._db._debug&&this._db._debug[b]||this._debug&&this._debug.all:this._debug&&this._debug.all},function(b,a){return void 0!==b?void 0!==a?(this._debug=this._debug||{},this._debug[b]=a,this.chainSend("debug",this._debug),this):this._debug&&this._debug[a]||this._db&&this._db._debug&&this._db._debug[b]:this._debug&&this._debug.all}]),on:new d({"string, function":function(b,a){this._listeners=this._listeners||{};this._listeners[b]=
