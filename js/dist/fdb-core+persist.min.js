...
h[d],void 0));e++);if(a){if("or"===k)return!0}else g=!1}else g=!1;if("and"===k&&!g)return!1}return g};d.prototype._matchOp=function(a,h,k){switch(a){case "$gt":return h>k[a];case "$gte":return h>=k[a];case "$lt":return h<k[a];case "$lte":return h<=k[a];case "$exists":return void 0===h!==k[a];case "$ne":return h!=k[a];case "$or":for(var e=0;e<k[a].length;e++)if(this._match(h,k[a][e],"and"))return!0;return!1;case "$and":for(e=0;e<k[a].length;e++)if(!this._match(h,k[a][e],"and"))return!1;return!0;case "$in":if(k[a]instanceof
Array){a=k[a];k=a.length;for(e=0;e<k;e++)if(a[e]===h)return!0;return!1}throw'ForerunnerDB.Collection "'+this.name()+'": Cannot use an $in operator on a non-array key: '+a;case "$nin":if(k[a]instanceof Array){a=k[a];k=a.length;for(e=0;e<k;e++)if(a[e]===h)return!1;return!0}throw'ForerunnerDB.Collection "'+this.name()+'": Cannot use a $nin operator on a non-array key: '+a;}return-1};d.prototype.count=function(a,h){return a?this.find(a,h).length:this._data.length};d.prototype.findSub=function(a,h,e,d){var g=
new p(h);a=this.find(a);var b=a.length,q,f,l=this._db.collection("__FDB_temp_"+this.objectId()),n={parents:b,subDocTotal:0,subDocs:[],pathFound:!1,err:""};for(q=0;q<b;q++)if(f=g.value(a[q])[0]){l.setData(f);f=l.find(e,d);if(d.returnFirst&&f.length)return f[0];n.subDocs.push(f);n.subDocTotal+=f.length;n.pathFound=!0}l.drop();if(d.noStats)return n.subDocs;n.pathFound||(n.err="No objects found in the parent documents with a matching path of: "+h);return n};d.prototype.insertIndexViolation=function(a){var h,
e=this._indexByName,d,g;if(this._primaryIndex.get(a[this._primaryKey]))h=this._primaryIndex;else for(d in e)if(e.hasOwnProperty(d)&&(g=e[d],g.unique()&&g.violation(a))){h=g;break}return h?h.name():!1};d.prototype.ensureIndex=function(a,h){this._indexByName=this._indexByName||{};this._indexById=this._indexById||{};var k,d={start:(new Date).getTime()};if(h)switch(h.type){case "hashed":k=new e(a,h,this);break;case "btree":k=new g(a,h,this);break;default:k=new e(a,h,this)}if(this._indexByName[k.name()])return{err:"Index with that name already exists"};
...
g));g.push(a);this._size++};f.prototype.remove=function(a,e){var g=this._unique,b=this._itemKeyHash(a,this._keys),d;g&&(g=this._itemHash(a,this._keys),delete this._uniqueLookup[g]);g=this._btree.get(b);void 0!==g&&(d=g.indexOf(a),-1<d&&(1===g.length?this._btree.del(b):g.splice(d,1),this._size--))};f.prototype.violation=function(a){a=this._itemHash(a,this._keys);return Boolean(this._uniqueLookup[a])};f.prototype.hashViolation=function(a){return Boolean(this._uniqueLookup[a])};f.prototype.lookup=function(a){return this._data[this._itemHash(a,
...
var a,f,e,g;if(b instanceof Array){f=b.length;g=[];for(a=0;a<f;a++)(e=this._data[b[a]])&&g.push(e);return g}if(b instanceof RegExp){g=[];for(a in this._data)this._data.hasOwnProperty(a)&&b.test(a)&&g.push(this._data[a]);return g}if("object"===typeof b){if(b.$ne){g=[];for(a in this._data)this._data.hasOwnProperty(a)&&a!==b.$ne&&g.push(this._data[a]);return g}if(b.$in&&b.$in instanceof Array){g=[];for(a in this._data)this._data.hasOwnProperty(a)&&-1<b.$in.indexOf(a)&&g.push(this._data[a]);return g}if(b.$nin&&
