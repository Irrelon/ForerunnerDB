...
delete this._links[g],this._linked--,this.debug()&&console.log('ForerunnerDB.AutoBind: Removed binding collection "'+this.name()+'" to output target: '+b),this;console.log("Cannot remove link, one does not exist to the target: "+b+" with the template: "+a)}else throw"Cannot data-bind without jQuery, please add jQuery to your page!";return this};b.prototype._dataReplace=function(b){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Replacing some data in document for collection "'+this.name()+
'"'),jQuery.observable(this._data).refresh(b)):hb.apply(this,arguments)};b.prototype._dataInsertAtIndex=function(b,a){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Inserting some data in document sub-array for collection "'+this.name()+'"'),jQuery.observable(this._data).insert(b,a)):v.apply(this,arguments)};b.prototype._dataRemoveAtIndex=function(b){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Removing some data from document sub-array for collection "'+this.name()+
'"'),jQuery.observable(this._data).remove(b)):ea.apply(this,arguments)};b.prototype._updateProperty=function(b,a,g){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Setting document property "'+a+'" for collection "'+this.name()+'"'),jQuery.observable(b).setProperty(a,g)):k.apply(this,arguments)};b.prototype._updateIncrement=function(b,a,g){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Incrementing document property "'+a+'" for collection "'+this.name()+'"'),jQuery.observable(b).setProperty(a,
b[a]+g)):P.apply(this,arguments)};b.prototype._updateSpliceMove=function(b,a,g){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Moving document array index from "'+a+'" to "'+g+'" for collection "'+this.name()+'"'),jQuery.observable(b).move(a,g)):S.apply(this,arguments)};b.prototype._updateSplicePush=function(b,a,g){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Pushing item into document sub-array for collection "'+this.name()+'"'),b.length>a?jQuery.observable(b).insert(a,
g):jQuery.observable(b).insert(g)):H.apply(this,arguments)};b.prototype._updatePush=function(b,a){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Pushing item into document sub-array for collection "'+this.name()+'"'),jQuery.observable(b).insert(a)):ja.apply(this,arguments)};b.prototype._updatePull=function(b,a){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Pulling item from document sub-array for collection "'+this.name()+'"'),jQuery.observable(b).remove(a)):L.apply(this,
arguments)};b.prototype._updateMultiply=function(b,a,g){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Multiplying value for collection "'+this.name()+'"'),jQuery.observable(b).setProperty(a,b[a]*g)):qa.apply(this,arguments)};b.prototype._updateRename=function(b,a,g){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Renaming property "'+a+'" to "'+g+'" on document for collection "'+this.name()+'"'),jQuery.observable(b).setProperty(g,b[a]),jQuery.observable(b).removeProperty(a)):
oa.apply(this,arguments)};b.prototype._updateUnset=function(b,a){this._linked?(this.debug()&&console.log('ForerunnerDB.AutoBind: Removing property "'+a+'" from document for collection "'+this.name()+'"'),jQuery.observable(b).removeProperty(a)):$.apply(this,arguments)};b.prototype._updatePop=function(b,a){var g,v=!1;this._linked?0<b.length&&(this.debug()&&console.log('ForerunnerDB.AutoBind: Popping item from sub-array in document for collection "'+this.name()+'"'),1===a?g=b.length-1:-1===a&&(g=0),
-1<g&&(jQuery.observable(arr).remove(g),v=!0)):v=aa.apply(this,arguments);return v}};k.extendView=function(b){var a=b.prototype.init;b.prototype.init=function(){this._linked=0;a.apply(this,arguments)};b.prototype.isLinked=function(){return this.data().isLinked()};b.prototype.link=function(b,a){var g=this.publicData();this.debug()&&console.log('ForerunnerDB.AutoBind: Setting up data binding on view "'+this.name()+'" in underlying (internal) view collection "'+g.name()+'" for output target: '+b);g.link(b,
a);return this};b.prototype.unlink=function(b,a){var g=this.publicData();this.debug()&&console.log('ForerunnerDB.AutoBind: Removing data binding on view "'+this.name()+'" in underlying (internal) view collection "'+g.name()+'" for output target: '+b);g.unlink(b,a);return this}};k.extendOverview=function(b){b.prototype.isLinked=function(){return this.data().isLinked()};b.prototype.link=function(b,a){this._data.link.apply(this._data,arguments);this._refresh()};b.prototype.unlink=function(b,a){this._data.unlink.apply(this._data,
arguments);this._refresh()}};k.extendDocument=function(b){b.prototype.isLinked=function(){return Boolean(this._linked)};b.prototype.link=function(b,a){if(window.jQuery){this._links=this._links||{};this._linked||(this._linked=0);var v,k;a&&"object"===typeof a?a.template&&"string"===typeof a.template&&(v=this.objectId(a.template),k=a.template):v=a;if(!this._links[v]){if(jQuery(b).length){if(!jQuery.templates[v]){if(!k)if(k=jQuery(a),k.length)k=jQuery(k[0]).html();else throw"Unable to bind document to target because template does not exist: "+
a;jQuery.views.templates(v,k)}jQuery.templates[v].link(b,this._data);this._links[v]=b;this._linked++;this.debug()&&console.log('ForerunnerDB.AutoBind: Added binding document "'+this.name()+'" to output target: '+b);return this}throw'Cannot bind view data to output target selector "'+b+'" because it does not exist in the DOM!';}throw"Cannot create a duplicate link to the target: "+b+" with the template: "+v;}throw"Cannot data-bind without jQuery, please add jQuery to your page!";};b.prototype.unlink=
function(b,a){if(window.jQuery){this._links=this._links||{};var v;a&&"object"===typeof a?a.template&&"string"===typeof a.template&&(v=this.objectId(a.template)):v=a;if(this._links[v])return jQuery.templates[v].unlink(b),delete this._links[v],this._linked--,this.debug()&&console.log('ForerunnerDB.AutoBind: Removed binding document "'+this.name()+'" to output target: '+b),this;console.log("Cannot remove link, one does not exist to the target: "+b+" with the template: "+a)}else throw"Cannot data-bind without jQuery, please add jQuery to your page!";
}};if("undefined"!==typeof jQuery)if(H("../lib/vendor/jsviews"),"undefined"!==typeof jQuery.views){H=["Collection","View","Overview","Document"];var a;for(a=0;a<H.length;a++)L.moduleFinished(H[a],function(b,a){if(k["extend"+b])k["extend"+b](a)});L.finishModule("AutoBind")}else throw"AutoBind plugin cannot continue because jsViews is not loaded - check your error log for url errors.";else throw"Cannot use AutoBind plugin without jQuery - please ensure jQuery is loaded first!";L.finishModule("AutoBind");
$.exports=k},{"../lib/vendor/jsviews":3}],3:[function(H,$,L){window.jQuery&&(this.jQuery=window.jQuery,function(){(function(k,a,b){function g(b,p){for(var a in p.props)R.test(a)&&(b[a]=p.props[a])}function H(b){return b}function v(b){debugger;return b}function ea(b){M=(Ca._dbgMode=b)?"Unavailable (nested view): use #getIndex()":"";Da("dbg",Pa.dbg=pa.dbg=b?v:H)}function L(b){this.name=(u.link?"JsViews":"JsRender")+" Error";this.message=b||this.name}function P(b,p){for(var a in p)b[a]=p[a];return b}
function S(b){return"function"===typeof b}function $(b,p,a){if(!wa.rTag||b)J=b?b.charAt(0):J,z=b?b.charAt(1):z,t=p?p.charAt(0):t,U=p?p.charAt(1):U,W=a||W,b="\\"+J+"(\\"+W+")?\\"+z,p="\\"+t+"\\"+U,A="(?:(?:(\\w+(?=[\\/\\s\\"+t+"]))|(?:(\\w+)?(:)|(>)|!--((?:[^-]|-(?!-))*)--|(\\*)))\\s*((?:[^\\"+t+"]|\\"+t+"(?!\\"+U+"))*?)",wa.rTag=A+")",A=RegExp(b+A+"(\\/)?|(?:\\/(\\w+)))"+p,"g"),s=RegExp("<.*>|([^\\\\]|^)[{}]|"+b+".*"+p);return[J,z,t,U,W]}function ja(ga,p){p||(p=ga,ga=b);var a,c,x,f;x=this;a=!p||"root"===
p;if(ga){if(f=x.type===p?x:b,!f)if(a=x.views,x._.useKey)for(c in a){if(f=a[c].get(ga,p))break}else for(c=0,x=a.length;!f&&c<x;c++)f=a[c].get(ga,p)}else if(a)for(;x.parent.parent;)f=x=x.parent;else for(;x&&!f;)f=x.type===p?x:b,x=x.parent;return f}function Aa(){var ga=this.get("item");return ga?ga.index:b}function qa(){return this.index}function oa(ga){var p,a=this,c=a.linkCtx,x=(a.ctx||{})[ga];x===b&&c&&c.ctx&&(x=c.ctx[ga]);x===b&&(x=Pa[ga]);x&&S(x)&&!x._wrp&&(p=function(){return x.apply(this&&this!==
k?this:a,arguments)},p._wrp=!0,P(p,x));return p||x}function Ra(ga,p,a,c){var x,f,h=+a===a&&p.tmpl.bnds[a-1],e=p.linkCtx;a=(c=c!==b&&{props:{},args:[c]})||(h?h(p.data,p,K):a);f=a.args[0];if(ga||h)x=e&&e.tag,x||(x={_:{inline:!e,bnd:h},tagName:":",cvt:ga,flow:!0,tagCtx:a,_is:"tag"},e&&(e.tag=x,x.linkCtx=e,a.ctx=O(a.ctx,e.view.ctx)),wa._lnk(x)),x._er=c&&f,g(x,a),a.view=p,x.ctx=a.ctx||{},delete a.ctx,p._.tag=x,f=aa(x,x.convert||"true"!==ga&&ga)[0],f=h&&p._.onRender?p._.onRender(f,p,h):f,p._.tag=b;return f!=
b?f:""}function aa(b,p){var a=b.tagCtx,c=a.view,x=a.args;p=p&&(""+p===p?c.getRsc("converters",p)||ha("Unknown converter: '"+p+"'"):p);x=x.length||a.index?p?x.slice():x:[c.data];p&&(p.depends&&(b.depends=wa.getDeps(b.depends,b,p.depends,p)),x[0]=p.apply(b,x));return x}function La(a,p){for(var c,f=this;c===b&&f;)c=(c=f.tmpl[a])&&c[p],f=f.parent;return c||K[a][p]}function Ma(a,p,c,f,x,e){var h,d,m,u,l,t,F,V,v,s="",k=p.linkCtx||0,z=p.ctx,A=c||p.tmpl,w=+f===f&&A.bnds[f-1];"tag"===a._is&&(h=a,a=h.tagName,
f=h.tagCtxs);h=h||k.tag;f=(e=e!==b&&(s+=e,[{props:{},args:[]}]))||(w?w(p.data,p,K):f);t=f.length;for(l=0;l<t;l++){l||c&&h||(V=p.getRsc("tags",a)||ha("Unknown tag: {{"+a+"}}"));d=f[l];if(!k.tag||h._er)c=d.tmpl,c=d.content=c&&A.tmpls[c-1],P(d,{tmpl:(h?h:V).template||c,render:ta,index:l,view:p,ctx:O(d.ctx,z)});if(c=d.props.tmpl)c=""+c===c?p.getRsc("templates",c)||C(c):c,d.tmpl=c;h||(V._ctr?(h=new V._ctr,v=!!h.init):wa._lnk(h={render:V.render}),h._={inline:!k},k&&(k.tag=h,h.linkCtx=k),(h._.bnd=w||k.fn)?
h._.arrVws={}:h.dataBoundOnly&&ha("{^{"+a+"}} tag must be data-bound"),h.tagName=a,h.parent=u=z&&z.tag,h._is="tag",h._def=V,h.tagCtxs=f);d.tag=h;h.dataMap&&h.tagCtxs&&(d.map=h.tagCtxs[l].map);h.flow||(F=d.ctx=d.ctx||{},d=h.parents=F.parentTags=z&&O(F.parentTags,z.parentTags)||{},u&&(d[u.tagName]=u),d[h.tagName]=F.tag=h)}p._.tag=h;if(!(h._er=e)){g(h,f[0]);h.rendering={};for(l=0;l<t;l++){d=h.tagCtx=h.tagCtxs[l];u=d.props;a=aa(h,h.convert);if(f=u.dataMap||h.dataMap)if(a.length||u.dataMap){e=d.map;if(!e||
e.src!==a[0]||x)e&&e.src&&e.unmap(),e=d.map=f.map(a[0],u);a=[e.tgt]}h.ctx=d.ctx;!l&&v&&(f=h.template,h.init(d,k,h.ctx),v=b,h.template!==f&&(h._.tmpl=h.template),k&&(k.attr=h.attr=k.attr||h.attr));f=b;h.render&&(f=h.render.apply(h,a));a=a.length?a:[p];f=f!==b?f:d.render(a[0],!0)||(x?b:"");s=s?s+(f||""):f}delete h.rendering}h.tagCtx=h.tagCtxs[0];h.ctx=h.tagCtx.ctx;h._.inline&&(m=h.attr)&&m!==B&&(s="text"===m?pa.html(s):"");return w&&p._.onRender?p._.onRender(s,p,w):s}function D(b,a,c,f,x,e,h,d){var g=
"array"===a,m={key:0,useKey:g?0:1,id:""+E++,onRender:d,bnds:{}};this.data=f;this.tmpl=x;this.content=h;this.views=g?[]:{};this.parent=c;this.type=a||"top";this._=m;this.linked=!!d;c?(a=c.views,f=c._,f.useKey?(a[m.key="_"+f.useKey++]=this,this.index=M,this.getIndex=Aa,e=f.tag,m.bnd=g&&(!e||!!e._.bnd&&e)):a.splice(m.key=this.index=e,0,this),this.ctx=b||c.ctx):this.ctx=b}function ba(b){var a,c,f,x,e,h;for(a in ma)if(x=ma[a],(e=x.compile)&&(c=b[a+"s"]))for(f in c)(x=c[f]=e(f,c[f],b))&&(h=wa.onStore[a])&&
h(f,x,e)}function Fa(a,c,f){S(c)?c={depends:c.depends,render:c}:(c.baseTag&&(c.flow=!!c.flow,c=P(P({},c.baseTag),c)),(a=c.template)!==b&&(c.template=""+a===a?C[a]||C(a):a),!1!==c.init&&(a=c._ctr=function(){},(a.prototype=c).constructor=a));f&&(c._parentTmpl=f);return c}function Ga(c,p,f,e){function x(p){if(""+p===p||0<p.nodeType){try{h=0<p.nodeType?p:!s.test(p)&&a&&a(k.document).find(p)[0]}catch(x){}h&&(p=C[c=c||h.getAttribute(X)],p||(c=c||"_"+ia++,h.setAttribute(X,c),p=C[c]=Ga(c,h.innerHTML,f,e)),
h=b);return p}}var d,h;p=p||"";d=x(p);e=e||(p.markup?p:{});e.tmplName=c;f&&(e._parentTmpl=f);!d&&p.markup&&(d=x(p.markup))&&d.fn&&(d.debug!==p.debug||d.allowCode!==p.allowCode)&&(d=d.markup);if(d!==b)return c&&!f&&(Sa[c]=function(){return p.render.apply(p,arguments)}),d.fn||p.fn?d.fn&&(p=c&&c!==d.tmplName?O(e,d):d):(p=ra(d,e),Ka(d.replace(w,"\\$&"),p)),ba(e),p}function Oa(b){function a(c,p){this.tgt=b.getTgt(c,p)}S(b)&&(b={getTgt:b});b.baseMap&&(b=P(P({},b.baseMap),b));b.map=function(b,c){return new a(b,
c)};return b}function ra(b,a){var c,f=Ca.wrapMap||{},e=P({markup:b,tmpls:[],links:{},tags:{},bnds:[],_is:"template",render:Y},a);a.htmlTag||(c=Qa.exec(b),e.htmlTag=c?c[1].toLowerCase():"");(c=f[e.htmlTag])&&c!==f.div&&(e.markup=u.trim(e.markup));return e}function sa(a,c){function f(d,g,h){var m,u,l;if(d&&"object"===typeof d&&!d.nodeType&&!d.markup&&!d.getTgt){for(u in d)f(u,d[u],g);return K}g===b&&(g=d,d=b);d&&""+d!==d&&(h=g,g=d,d=b);l=h?h[e]=h[e]||{}:f;u=c.compile;null===g?d&&delete l[d]:(g=u?g=
u(d,g,h):g,d&&(l[d]=g));u&&g&&(g._is=a);g&&(m=wa.onStore[a])&&m(d,g,u);return g}var e=a+"s";K[e]=f;ma[a]=c}function xa(b,a,c){var f=this.jquery&&(this[0]||ha('Unknown template: "'+this.selector+'"')),d=f.getAttribute(X);return Y.call(d?C[d]:C(f),b,a,c)}function Na(b,a,c){if(Ca._dbgMode)try{return b.fn(a,c,K)}catch(f){return ha(f,c)}return b.fn(a,c,K)}function Y(b,a,c,f,d,e){return f||!this.fn._nvw||u.isArray(b)?ta.call(this,b,a,c,f,d,e):Na(this,b,{tmpl:this})}function ta(a,c,f,d,e,g){var h,m,l,k,
s,t,F,V,v;l=this;var z="";!!c===c&&(f=c,c=b);!0===e&&(h=!0,e=0);l.tag?(t=l,l=l.tag,m=l._,s=l.tagName,V=m.tmpl||t.tmpl,v=l.attr&&l.attr!==B,c=O(c,l.ctx),F=t.content,!1===t.props.link&&(c=c||{},c.link=!1),d=d||t.view,a=arguments.length?a:d):V=l;if(V&&(!d&&a&&"view"===a._is&&(d=a),d&&(F=F||d.content,g=g||d._.onRender,a===d&&(a=d.data),c=O(c,d.ctx)),d&&"top"!==d.type||((c=c||{}).root=a),V.fn||(V=C[V]||C(V)),V)){t=g=!1!==(c&&c.link)&&!v&&g;!0===g&&(t=b,g=d._.onRender);c=V.helpers?O(V.helpers,c):c;if(u.isArray(a)&&
!f)for(k=h?d:e!==b&&d||new D(c,"array",d,a,V,e,F,g),h=0,m=a.length;h<m;h++)l=a[h],s=new D(c,"item",k,l,V,(e||0)+h,F,g),l=Na(V,l,s),z+=k._.onRender?k._.onRender(l,s):l;else{if(d||!V.fn._nvw)k=h?d:new D(c,s||"data",d,a,V,e,F,g),m&&!l.flow&&(k.tag=l);z+=Na(V,a,k)}return t?t(z,k):z}return""}function ha(b,a,c){c=Ca.onError(b,a,c);if(""+b===b)throw new wa.Err(c);return!a.linkCtx&&a.linked?pa.html(c):c}function ca(b){ha("Syntax error\n"+b)}function Ka(b,a,f,g){function x(a){(a-=t)&&F.push(b.substr(t,a).replace(N,
"\\n"))}function l(a){a&&ca('Unmatched or missing tag: "{{/'+a+'}}" in template:\n'+b)}var h,m,u,O=a&&a.allowCode,k=[],t=0,s=[],F=k,z=[,,k];f&&(b=J+b+U);l(s[0]&&s[0][2].pop()[0]);b.replace(A,function(c,d,h,k,v,w,A,D,E,Q,J,K){w&&(v=":",k=B);Q=Q||f;d=(d||f)&&[[]];var C="",r="",q="",n="",y="",ua="",na="",Ta="";A=!Q&&!v&&!A;h=h||(E=E||"#data",v);x(K);t=K+c.length;D?O&&F.push(["*","\n"+E.replace(G,"$1")+"\n"]):h?("else"===h&&(T.test(E)&&ca('for "{{else if expr}}" use "{{else expr}}"'),d=z[7],z[8]=b.substring(z[8],
K),z=s.pop(),F=z[2],A=!0),E&&(e(E.replace(N," "),d,a).replace(Ba,function(b,a,c,f,d,e,p,h){p?(r+=e+",",n+="'"+h+"',"):c?(q+=f+e+",",ua+=f+"'"+h+"',"):a?na+=e:("trigger"===d&&(Ta+=e),C+=f+e+",",y+=f+"'"+h+"',",u=u||R.test(d));return""}).slice(0,-1),d&&d[0]&&d.pop()),m=[h,k||!!g||u||"",A&&[],[n.slice(0,-1),y.slice(0,-1),ua.slice(0,-1)],[r.slice(0,-1),C.slice(0,-1),q.slice(0,-1)],na,Ta,d||0],F.push(m),A&&(s.push(z),z=m,z[8]=t)):J&&(l(J!==z[0]&&"else"!==z[0]&&J),z[8]=b.substring(z[8],K),z=s.pop());l(!z&&
J);F=z[2]});x(b.length);(t=k[k.length-1])&&l(""+t!==t&&+t[8]===t[8]&&t[0]);f?(h=d(k,b,f),c(h,k[0][7])):h=d(k,a);h._nvw&&(h._nvw=!/[~#]/.test(b));return h}function c(b,a){b.deps=[];for(var c in a)"_jsvto"!==c&&a[c].length&&(b.deps=b.deps.concat(a[c]));b.paths=a}function f(b,a){return"\n\t"+(a?a+":{":"")+"args:["+b[0]+"]"+(b[1]||!a?",\n\tprops:{"+b[1]+"}":"")+(b[2]?",\n\tctx:{"+b[2]+"}":"")}function e(b,a,c){var f,d,e,h,g,l=a&&a[0],m=0,u=c?c.links:l&&(l.links=l.links||{}),k={},s={0:-1},O=0;return(b+
(c?" ":"")).replace(/\)\^/g,").").replace(Ha,function(F,v,A,B,w,E,D,Q,J,K,N,C,G,M,r,q,n,y,ua,na){function Ta(r,q,b,n,c,y,ua,na){return b&&(l&&("linkTo"===f&&(d=a._jsvto=a._jsvto||[],d.push(w)),f&&!e||l.push(w.slice(q.length))),"."!==b)?(r=(n?'view.hlp("'+n+'")':c?"view":"data")+(na?(y?"."+y:n?"":c?"":"."+b)+(ua||""):(na=n?"":c?y||"":b,"")),r+=na?"."+na:"",q+("view.data"===r.slice(0,9)?r.slice(5):r)):r}E=E||"";A=A||v||C;w=w||J;K=K||n||"";J=na.length-1;if(!D||g||h)return!l||!q||g||h||f&&!e&&!d||(q=
s[O],J>ua-q&&(q=na.slice(q,ua+1),q=z+":"+q+" onerror=''"+t,D=u[q],D||(u[q]=!0,u[q]=D=Ka(q,c||l,!0),D.paths.push({_jsvOb:D})),!0!==D&&(d||l).push({_jsvOb:D}))),g?(g=!G,g?F:'"'):h?(h=!M,h?F:'"'):(A?(O++,s[O]=ua++,A):"")+(y?O?"":(m=na.slice(m,ua),f?(f=e=d=!1,"\b"):"\b,")+m+(m=ua+F.length,l&&a.push(l=[]),"\b"):Q?(O&&ca(b),l&&a.pop(),f=w,e=B,m=ua+F.length,B&&(l=a[f]=[]),w+":"):w?w.split("^").join(".").replace(la,Ta)+(K?(k[++O]=!0,"."!==w.charAt(0)&&(s[O]=ua),K):E):E?E:r?(k[O--]=!1,r)+(K?(k[++O]=!0,K):
""):N?(k[O]||ca(b),","):v?"":(g=G,h=M,'"'));ca(b)})}function d(a,e,g){var l,m,u,h,k,O,t,z,F,s,v,w,A,D,E,K,J,Q,C,G,M,ma,P,U,W,r,q=0,n="",y={},ua=a.length;""+e===e?(Q=g?'data-link="'+e.replace(N," ").slice(1,-1)+'"':e,e=0):(Q=e.tmplName||"unnamed",e.allowCode&&(y.allowCode=!0),e.debug&&(y.debug=!0),v=e.bnds,J=e.tmpls);for(l=0;l<ua;l++)if(m=a[l],""+m===m)n+='\n+"'+m+'"';else if(u=m[0],"*"===u)n+=";\n"+m[1]+"\nret=ret";else{h=m[1];M=m[2];k=f(m[3],"params")+"},"+f(A=m[4]);U=m[5];r=m[6];E=m[8];(D="else"===
u)||(q=0,v&&(w=m[7])&&(q=v.push(w)));(m=":"===u)?h&&(u=h===B?">":h+u):(M&&(C=ra(E,y),C.tmplName=Q+"/"+u,d(M,C),J.push(C)),D||(G=u,ma=n,n=""),C=(C=a[l+1])&&"else"===C[0]);W=U?";\ntry{\nret+=":"\n+";E=D="";if(m&&(w||r||h&&h!==B)){P="return {"+k+"};";K='c("'+h+'",view,';P=new Function("data,view,j,u"," // "+Q+" "+q+" "+u+"\n"+P);P._er=U;D=K+q+",";E=")";P._tag=u;if(g)return P;c(P,w);s=!0}n+=m?(g?(U?"\ntry{\n":"")+"return ":W)+(s?(s=b,F=z=!0,K+(w?(v[q-1]=P,q):"{"+k+"}")+")"):">"===u?(t=!0,"h("+A[0]+")"):
"((v="+A[0]+')!=null?v:"")'):(F=O=!0,"\n{view:view,tmpl:"+(M?J.length:"0")+","+k+"},");if(G&&!C){n="["+n.slice(0,-1)+"]";K='t("'+G+'",view,this,';if(g||w){n=new Function("data,view,j,u"," // "+Q+" "+q+" "+G+"\nreturn "+n+";");n._er=U;n._tag=u;w&&c(v[q-1]=n,w);if(g)return n;D=K+q+",undefined,";E=")"}n=ma+W+K+(q||n)+")";G=w=0}U&&(F=!0,n+=";\n}catch(e){ret"+(g?"urn ":"+=")+D+"j._err(e,view,"+U+")"+E+";}\n"+(g?"":"ret=ret"))}n="// "+Q+"\nvar v"+(O?",t=j._tag":"")+(z?",c=j._cnvt":"")+(t?",h=j.converters.html":
"")+(g?";\n":',ret=""\n')+(y.debug?"debugger;":"")+n+(g?"\n":";\nreturn ret;");try{n=new Function("data,view,j,u",n)}catch(na){ca("Compiled template code:\n\n"+n+'\n: "'+na.message+'"')}e&&(e.fn=n);F||(n._nvw=!0);return n}function O(b,a){return b&&b!==a?a?P(P({},a),b):b:a&&P({},a)}function F(b){return fa[b]||(fa[b]="&#"+b.charCodeAt(0)+";")}function l(b){var a,c,f=[];if("object"===typeof b)for(a in b)(c=b[a])&&c.toJSON&&!c.toJSON()||S(c)||f.push({key:a,prop:c});return f}function m(b){return null!=
b?Za.test(b)&&(""+b).replace(Z,F)||b:""}if(!(a&&a.render||k.jsviews)){var u,Q,A,s,M,J="{",z="{",t="}",U="}",W="^",la=/^(!*?)(?:null|true|false|\d[\d.]*|([\w$]+|\.|~([\w$]+)|#(view|([\w$]+))?)([\w$.^]*?)(?:[.[^]([\w$]+)\]?)?)$/g,Ha=/(\()(?=\s*\()|(?:([([])\s*)?(?:(\^?)(!*?[#~]?[\w$.^]+)?\s*((\+\+|--)|\+|-|&&|\|\||===|!==|==|!=|<=|>=|[<>%*:?\/]|(=))\s*|(!*?[#~]?[\w$.^]+)([([])?)|(,\s*)|(\(?)\\?(?:(')|("))|(?:\s*(([)\]])(?=\s*\.|\s*\^|\s*$)|[)\]])([([]?))|(\s+)/g,N=/[ \t]*(\r\n|\n|\r)/g,G=/\\(['"])/g,
w=/['"\\]/g,Ba=/(?:\x08|^)(onerror:)?(?:(~?)(([\w$]+):)?([^\x08]+))\x08(,)?([^\x08]+)/gi,T=/^if\s/,Qa=/<(\w+)[>\s]/,Za=/[\x00`><\"'&]/,R=/^on[A-Z]|^convert(Back)?$/,Z=/[\x00`><"'&]/g,ia=0,E=0,fa={"&":"&amp;","<":"&lt;",">":"&gt;","\x00":"&#0;","'":"&#39;",'"':"&#34;","`":"&#96;"},B="html",X="data-jsv-tmpl",Sa={},ma={template:{compile:Ga},tag:{compile:Fa},helper:{},converter:{}},K={jsviews:"v1.0.0-beta",settings:function(b){P(Ca,b);ea(Ca._dbgMode);Ca.jsv&&Ca.jsv()},sub:{View:D,Err:L,tmplFn:Ka,cvt:aa,
parse:e,extend:P,syntaxErr:ca,onStore:{},_lnk:H,_ths:g},map:Oa,_cnvt:Ra,_tag:Ma,_err:ha};(L.prototype=Error()).constructor=L;Aa.depends=function(){return[this.get("item"),"index"]};qa.depends=function(){return["index"]};D.prototype={get:ja,getIndex:qa,getRsc:La,hlp:oa,_is:"view"};for(Q in ma)sa(Q,ma[Q]);var C=K.templates,pa=K.converters,Pa=K.helpers,Da=K.tags,wa=K.sub,Ca=K.settings;a?(u=a,u.fn.render=xa,u.observable&&(P(wa,u.views.sub),K.map=u.views.map)):(u=k.jsviews={},u.isArray=Array&&Array.isArray||
function(b){return"[object Array]"===Object.prototype.toString.call(b)});u.render=Sa;u.views=K;u.templates=C=K.templates;Ca({debugMode:ea,delimiters:$,onError:function(a,c,f){c&&(a=f===b?"{Error: "+a+"}":S(f)?f(a,c):f);return a==b?"":a},_dbgMode:!0});Da({"else":function(){},"if":{render:function(b){return this.rendering.done||!b&&(arguments.length||!this.tagCtx.index)?"":(this.rendering.done=!0,this.selected=this.tagCtx.index,this.tagCtx.render(this.tagCtx.view,!0))},onUpdate:function(b,a,c){var f;
for(b=0;(f=this.tagCtxs[b])&&f.args.length;b++)if(f=f.args[0],a=!f!==!c[b].args[0],!this.convert&&f||a)return a;return!1},flow:!0},"for":{render:function(a){var c,f=this.tagCtx,d="",e=0;if(!this.rendering.done){if(c=!arguments.length)a=f.view.data;a!==b&&(d+=f.render(a,c),e+=u.isArray(a)?a.length:1);if(this.rendering.done=e)this.selected=f.index}return d},flow:!0},include:{flow:!0},"*":{render:H,flow:!0}});Da("props",{baseTag:Da["for"],dataMap:Oa(l)});pa({html:m,attr:m,url:function(a){return a!=b?
encodeURI(""+a):null===a?a:""}});$()}})(this,this.jQuery);(function(k,a,b){function g(b){return D(b)?new v(b):new H(b)}function H(b){this._data=b;return this}function v(b){this._data=b;return this}function ea(b,a){b=D(b)?b:[b];var e,d,g=a,k=g,l=b.length,m=[];for(e=0;e<l;e++)d=b[e],Y(d)?m=m.concat(ea(d.call(a,a),a)):""+d!==d?(a=k=d,k!==g&&m.push(g=k)):(k!==g&&m.push(g=k),m.push(d));return m}function L(b,a){var e,d;for(e in b){d=!0;break}d||delete xa[a]}function P(a,f){if(!a.data||!a.data.off){var e,
d,g,k,l,m=f.oldValue,u=f.value,v=a.data,A=v.observeAll,s=!v.cb.noArray,M=v.paths;if(a.type===sa)(v.cb.array||v.cb).call(v,a,f);else if(v.prop===f.path||"*"===v.prop)k=typeof m===Fa&&(M[0]||s&&D(m)),l=typeof u===Fa&&(M[0]||s&&D(u)),A?(e=A._path+"."+f.path,d=A.filter,g=[a.target].concat(A.parents()),k&&ja(s,A.ns,[m],M,v.cb,!0,d,[g],e),l&&ja(s,A.ns,[u],M,v.cb,b,d,[g],e)):(k&&ja(s,[m],M,v.cb,!0),l&&ja(s,[u],M,v.cb)),v.cb(a,f)}}function S(){function c(b,c,f,d){var e,g=ca(B),l=D(B)?[B]:B;b=T?b+"."+T:b;
if(A||d)g&&a(l).off(b,P);else{if(t=g&&a._data(B))for(J=(t=(t=t&&t.events)&&t[f?sa:ra])&&t.length;J--;)if((z=t[J].data)&&z.cb._cId===s._cId&&z.ns===T){if(f)return;("*"===c&&z.prop!==c||z.prop===m)&&a(B).off(b,P)}c=f?{}:{fullPath:u,paths:c?[c]:[],prop:m};c.ns=T;c.cb=s;w&&(c.observeAll={_path:w,path:function(){e=G.length;return w.replace(/[[.]/g,function(b){e--;return"["===b?"["+a.inArray(G[e-1],G[e]):"."})},parents:function(){return G},filter:aa,ns:T});a(l).on(b,null,c,P);H&&((xa[s._cId]=H)[a.data(B,
"obId")||a.data(B,"obId",ta++)]=B)}}function f(b,a){b._ob=U(b,fa);var c=fa;return function(f,d){var g=b._ob,l=a.length;typeof g===Fa&&(e(g,!0),(l||R&&D(g))&&ja(R,[g],a,s,U,!0));g=b._ob=U(b,c);typeof g===Fa&&(e(g),(l||R&&D(g))&&ja(R,[g],a,s,U,[c]));s(f,d)}}function e(a,f,d,e){if(R){var l=B,m=w;B=a;e&&(B=a[e],w+="."+e);aa&&B&&(B=g._fltr(w,B,e?[a].concat(G):G,aa));B&&(d||D(B))&&c(sa+".observe"+(s?".obs"+(M=s._cId=s._cId||ha++):""),b,!0,f);B=l;w=m}}var d,k,v,l,m,u,Q,A,s,M,J,z,t,U,W,H,S,N,G,w,aa,T,ba,
$,R=!1!=this,Z=!0,ia=Na,E=Array.apply(0,arguments);d=E.pop();var fa=E.shift(),B=N=fa,X=E.length;fa+""===fa&&R&&(T=fa,fa=B=N=E.shift(),X--);if(Y(d))s=d;else if(d+""===d&&(w=d,G=E.pop(),aa=E.pop(),d=E.pop(),X-=3),!0===d?A=d:d&&(fa=d,Z=b),d=E[X-1],X&&d===b||Y(d))s=E.pop(),X--;X&&Y(E[X-1])&&(U=s,s=E.pop(),X--);ia+=A?s?".obs"+s._cId:"":".obs"+(M=s._cId=s._cId||ha++);A||(H=xa[M]=xa[M]||{});ba=T&&T.match(Oa)||[""];for($=ba.length;$--;)for(T=ba[$],D(N)?e(N,A,!0):A&&0===X&&N&&c(ia,""),d=S=0;d<X;d++)if(u=E[d],
""!==u){B=N;if(""+u===u){l=u.split("^");l[1]&&(S=l[0].split(".").length,u=l.join("."),S=u.split(".").length-S);if(U&&(W=U(u,N))){X+=W.length-1;Ma.apply(E,[d--,1].concat(W));continue}l=u.split(".")}else Z&&!Y(u)&&(u&&u._jsvOb&&(N=f(u,E.slice(d+1)),N.noArray=!R,N._cId=s._cId,ja(R,[fa],E.slice(0,d),N,U),N=b,u=u._ob),B=u),N=u,l=[N];for(;B&&(m=l.shift())!==b;)if(typeof B===Fa){if(""+m===m){if(""===m)continue;if(l.length<S+1&&!B.nodeType){if(!A&&(t=ca(B)&&a._data(B))){J=(t=(t=t.events)&&t[ra])&&t.length;
for(v=0;J--;)!(z=t[J].data)||z.cb!==s||z.ns!==T||z.prop!==m&&"*"!==z.prop||((k=l.join("."))&&z.paths.push(k),v++);if(v){B=B[m];continue}}if("*"===m){!A&&t&&t.length&&c(ia,"",!1,!0);Y(B)?(Q=B.depends)&&ja(R,[Q],s,A||fa):c(ia,"");for(k in B)e(B,A,b,k);break}else m&&c(ia+"."+m,l.join("^"))}w&&(w+="."+m);m=B[m]}if(Y(m)){(Q=m.depends)&&ja(R,[B],ea(Q,B),s,U,A||[fa]);break}B=m}e(B,A)}M&&L(H,M);return{cbId:M,bnd:H}}function $(){[].push.call(arguments,!0);return S.apply(this,arguments)}function ja(){var b=
[].concat.apply([],arguments);return S.apply(b.shift(),b)}function Aa(b,a,e,d){b+""!==b&&(e=a,a=b,b="");oa(b,this._data,a,e,[],"root",d)}function qa(b,a,e){Aa.call(this,b,a,e,!0)}function oa(a,f,e,d,k,v,l){function m(b,a){A=b.length;J=[[b]].concat(J);for(M=v+"[]";A--;)u(b,A,a,1)}function u(f,l,m,k){var u;l!==ba&&(u=g._fltr(M,f[l],J,d))&&oa(a,u,e,d||(k?b:0),J.slice(),M,m)}function H(b,a){var c=k;v=b.data.observeAll._path;J=[b.target].concat(k);switch(a.change){case "insert":m(a.items);break;case "remove":m(a.items,
!0);break;case "refresh":m(a.oldItems,!0);m(b.target);break;case "set":M=v+"."+a.path,u(a,"oldValue",!0),u(a,"value")}e.apply(this,arguments);k=c}var A,s,M,J;if(typeof f===Fa){s=D(f)?"":"*";if(e){if(s||0!==d)H._cId=e._cId=e._cId||ha++,S(a,f,s,H,l,d,k,v)}else S(a,f,s,b,l,d,k,v);if(s)for(A in J=[f].concat(k),f)M=v+"."+A,u(f,A,l);else m(f,l)}}function Ra(b){return 0>b.indexOf(".")&&0>b.indexOf("[")}if(!a)throw"jsViews/jsObservable require jQuery";if(!a.observable){k=a.views=a.views||{jsviews:"v1.0.0-alpha",
sub:{}};var aa=k.sub,La=a.event.special,Ma=[].splice,D=a.isArray,ba=a.expando,Fa="object",Ga=parseInt,Oa=/\S+/g,ra=aa.propChng=aa.propChng||"propertyChange",sa=aa.arrChng=aa.arrChng||"arrayChange",xa=aa._cbBnds=aa._cbBnds||{},Na=ra+".observe",Y=a.isFunction,ta=1,ha=1,ca=a.hasData,Ka={};aa.getDeps=function(){var b=arguments;return function(){for(var a,e,d=[],g=b.length;g--;)a=b[g--],(e=b[g])&&(d=d.concat(Y(e)?e(a,a):e));return d}};a.observable=g;g._fltr=function(b,a,e,d){if(d&&Y(d)?d(b,a,e):1)return a=
Y(a)?a.set&&a.call(e[0]):a,typeof a===Fa&&a};g.Object=H;g.Array=v;a.observe=g.observe=S;a.unobserve=g.unobserve=$;g._apply=ja;H.prototype={_data:null,observeAll:Aa,unobserveAll:qa,data:function(){return this._data},setProperty:function(a,f,e){var d,g=this._data;a=a||"";if(g)if(D(a))for(d=a.length;d--;)f=a[d],this.setProperty(f.name,f.value,e===b||e);else if(""+a!==a)for(d in a)this.setProperty(d,a[d],f);else if(a!==ba){for(a=a.split(".");g&&1<a.length;)g=g[a.shift()];g&&this._setProperty(g,a[0],f,
e)}return this},removeProperty:function(a){this.setProperty(a,Ka);return this},_setProperty:function(a,f,e,d){var g,k,l,m=f?a[f]:a;Y(m)&&m.set&&(k=m,g=!0===m.set?m:m.set,m=m.call(a));if(m!==e||d&&m!=e)if(!(m instanceof Date)||m>e||m<e)g?(g.call(a,e),e=k.call(a)):(l=e===Ka)?(delete a[f],e=b):f&&(a[f]=e),this._trigger(a,{change:"set",path:f,value:e,oldValue:m,remove:l})},_trigger:function(b,f){a(b).triggerHandler(ra,f)}};v.prototype={_data:null,observeAll:Aa,unobserveAll:qa,data:function(){return this._data},
insert:function(a,b){var e=this._data;1===arguments.length&&(b=a,a=e.length);a=Ga(a);-1<a&&a<=e.length&&(b=D(b)?b:[b],b.length&&this._insert(a,b));return this},_insert:function(a,b){var e=this._data,d=e.length;Ma.apply(e,[a,0].concat(b));this._trigger({change:"insert",index:a,items:b},d)},remove:function(a,f){var e;e=this._data;a===b&&(a=e.length-1);a=Ga(a);f=f?Ga(f):0===f?0:1;-1<f&&-1<a&&(e=e.slice(a,a+f),(f=e.length)&&this._remove(a,f,e));return this},_remove:function(a,b,e){var d=this._data,g=
...
this._data,d=e.length;Ma.apply(e,[0,e.length].concat(b));this._trigger({change:"refresh",oldItems:a},d)},_trigger:function(b,f){var e=this._data,d=e.length,e=a([e]);e.triggerHandler(sa,b);d!==f&&e.triggerHandler(ra,{change:"set",path:"length",value:d,oldValue:f})}};La[ra]=La[sa]={remove:function(b){var f,e,d,g,k=b.data;if(k&&(k.off=!0,k=k.cb)&&(f=xa[k._cId])){b=a._data(this).events[b.type];for(d=b.length;d--&&!e;)e=(g=b[d].data)&&g.cb===k;e||(delete f[a.data(this,"obId")],L(f,k._cId))}}};k.map=function(c){function f(a,
d,f){var k,l=this;this.src&&this.unmap();"object"===typeof a&&(l.src=a,l.tgt=f||l.tgt||[],l.options=d||l.options,l.update(),c.obsSrc&&g(l.src).observeAll(l.obs=function(a,d){k||(k=!0,c.obsSrc(l,a,d),k=b)},l.srcFlt),c.obsTgt&&g(l.tgt).observeAll(l.obt=function(a,d){k||(k=!0,c.obsTgt(l,a,d),k=b)},l.tgtFlt))}Y(c)&&(c={getTgt:c});c.baseMap&&(c=a.extend({},c.baseMap,c));c.map=function(a,b,c){return new f(a,b,c)};(f.prototype={srcFlt:c.srcFlt||Ra,tgtFlt:c.tgtFlt||Ra,update:function(a){g(this.tgt).refresh(c.getTgt(this.src,
