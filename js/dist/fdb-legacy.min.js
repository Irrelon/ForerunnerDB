...
e._querySettings.options)),c.insert.length||c.remove.length)){c.insert.length&&this.chainSend("insert",c.insert);if(c.update.length)for(d=e._privateData.primaryKey(),a=0;a<c.update.length;a++)b={},b[d]=c.update[a][d],this.chainSend("update",{query:b,update:c.update[a]});if(c.remove.length){d=e._privateData.primaryKey();b=[];var f={$or:b};for(a=0;a<c.remove.length;a++)b.push({_id:c.remove[a][d]});this.chainSend("remove",f)}return!0}return!1}});var c=b.find(this._querySettings.query,this._querySettings.options);
