...
(function () {
	var init = (function (ForerunnerDB) {
		 ...
		 * @returns {boolean}
		 */
		View.prototype.isBound = function (selector) {
		...
		View.prototype.bindSortDom = function (selector, itemArr) {
			...
			if (this._debug || (this._db && this._db._debug)) {
				console.log('ForerunnerDB.View.Bind: Sorting data in DOM...', itemArr);
			}
			...
			for (arrIndex = 0; arrIndex < itemArr.length; arrIndex++) {
				...
				if (domItem.length) {
					if (arrIndex === 0) {
						if (this._debug || (this._db && this._db._debug)) {
							console.log('ForerunnerDB.View.Bind: Sort, moving to index 0...', domItem);
						}
						container.prepend(domItem);
					} else {
						if (this._debug || (this._db && this._db._debug)) {
							console.log('ForerunnerDB.View.Bind: Sort, moving to index ' + arrIndex + '...', domItem);
						}
						domItem.insertAfter(container.children(':eq(' + (arrIndex - 1) + ')'));
					}
				} else {
					if (this._debug || (this._db && this._db._debug)) {
						console.log('ForerunnerDB.View.Bind: Warning, element for array item not found!', arrItem);
					}
				}
			}
		};
		...
		View.prototype.bindRefresh = function (obj) {
			...
			for (bindKey in binds) {
				if (binds.hasOwnProperty(bindKey)) {
					...
					if (this._debug || (this._db && this._db._debug)) { console.log('Bind: sorting DOM...'); }
		...
		View.prototype.processQueue = function (type, callback) {
		...
		View.prototype._bindChange = function (newDataArr) {
			if (this._debug || (this._db && this._db._debug)) {
				console.log('ForerunnerDB.View.Bind: Bind data change, refreshing bind...', newDataArr);
				console.log('Bind: change received');
			}
	...
	if (typeof(define) === 'function' && define.amd) {
		// Use AMD
		define(['require', '../ForerunnerDB', './ForerunnerDB.View'], function (require, ForerunnerDB, View) {
