(function () {
	var init = (function (ForerunnerDB) {
		...
		View.prototype.update = function (query, update) {
			// Modify transform data
			this._transformUpdate(query, update);

			return this._privateData.update(query, update);
		};
		...
		View.prototype.link = function (outputTargetSelector, templateSelector) {
			return this._privateData.link(outputTargetSelector, templateSelector);
		};

		View.prototype.unlink = function (outputTargetSelector, templateSelector) {
			return this._privateData.unlink(outputTargetSelector, templateSelector);
		};
		...
		View.prototype.refresh = function (force) {
			...
			if (this._privateData._linked) {
				// Update data and observers
				$.observable(this._privateData._data).refresh(sortedData);
			} else {
