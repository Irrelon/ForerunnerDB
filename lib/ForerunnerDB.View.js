	...
	var init = (function (ForerunnerDB) {
		...
		View.prototype.update = function (query, update) {
			...
			for (var i = 0; i < updates.length; i++) {
				...
				this._transformUpdate(tQuery, this._transformIn(item));
			}
		...
		View.prototype.link = function (outputTargetSelector, templateSelector) {
			return this.publicData().link(outputTargetSelector, templateSelector);
		};

		View.prototype.unlink = function (outputTargetSelector, templateSelector) {
			return this.publicData().unlink(outputTargetSelector, templateSelector);
		};
		...
		View.prototype.refresh = function (force) {
			...
			if (this._privateData._linked) {
				...
				$.observable(this.publicData()._data).refresh(sortedData);
			} else {
