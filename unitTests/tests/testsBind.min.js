...
function(){a.dbUp();a.viewUp();a.dataUp();a.domUp();userView.queryOptions({sort:{name:1}},!1).bind("#testTarget",{template:function(a,b){b('<li class="item" id="'+a._id+'">'+a.name+"</li>")}});user.insert({_id:"2342",name:"adam"});user.insert({_id:"23432",name:"zelda"});user.insert({_id:"2322",name:"beta"});userView.find();var b=$("#testTarget").find(".item");ok(6===b.length,"Insert documents");console.log($(b[0]).html(),$(b[1]).html(),$(b[2]).html(),$(b[3]).html(),$(b[4]).html(),$(b[5]).html());
ok("adam"===$(b[0]).html(),"Alphabetical 1");ok("beta"===$(b[1]).html(),"Alphabetical 2");ok("Dean"===$(b[2]).html(),"Alphabetical 3");ok("Jim"===$(b[3]).html(),"Alphabetical 4");ok("Kat"===$(b[4]).html(),"Alphabetical 5");ok("zelda"===$(b[5]).html(),"Alphabetical 6");a.viewDown();a.domDown();a.dbDown()})};"function"===typeof define&&define.amd?require(["../ForerunnerDB","./base"],function(d,a){c(d,a)}):c(ForerunnerDB,base)})();
